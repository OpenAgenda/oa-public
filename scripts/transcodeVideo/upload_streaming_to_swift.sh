...
echo ""
echo "Suppression du contenu de '$SWIFT_CONTAINER_NAME/$SWIFT_OBJECT_PREFIX'..."
if ! swift delete --prefix "$SWIFT_OBJECT_PREFIX" "$SWIFT_CONTAINER_NAME"; then
  echo "Erreur : Impossible de supprimer le contenu."
  exit 1
else
...
# On récupère la liste des objets (fichiers) correspondant au préfixe
OBJECT_LIST=$(swift list "$SWIFT_CONTAINER_NAME" --prefix "$SWIFT_OBJECT_PREFIX")

if [ -z "$OBJECT_LIST" ]; then
  echo "Aucun objet trouvé avec le préfixe '$SWIFT_OBJECT_PREFIX'. Aucune purge nécessaire."
else
