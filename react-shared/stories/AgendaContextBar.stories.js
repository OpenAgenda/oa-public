...
const mswHandlers = {
  basicEvents: rest.get('/events', async (_req, res, ctx) => res(ctx.json(eventsResponse))),
  manyEvents: rest.get('/events', async (req, res, ctx) => res(
    ctx.json(
      manyEventsResponse[
  ...
  drafts: rest.get('/drafts', async (_req, res, ctx) => res(ctx.json(draftsResponse))),
};
