var initMenu = function() {
    ...
    init_filter_calendar('.js_date_embed_filter', '.js_calendar_embed');
    ...
    init_widget_trigger_show();
    init_embed_trigger_show();
...
var init_embed_trigger_show = function(){

  $('.js_embed_advanced').triggerShow('.js_embed_advanced_menu', {
    onShow: function(){ 

      $(this.elem).parents('.js_menu_content').css('display', 'inline-block'); // ie8 tweak
    }
  });

};

var init_widget_trigger_show = function(){

  $('.js_widget_advanced').triggerShow('.js_widget_advanced_menu', {
    onShow: function(){ 
...
var init_embed_handler = function(){

  var embedHandler = new codeFieldHandler('.js_embed_code', {urlDom: 'iframe'});
  var facebookHandler = new codeFieldHandler('.js_facebook_code');

  var set_embed_parameter = function(inputCtl) {

    embedHandler.setParameter(inputCtl.elem.attr('name'), inputCtl.cleanValues[inputCtl.elem.attr('name')]);
    facebookHandler.setParameter(inputCtl.elem.attr('name'), inputCtl.cleanValues[inputCtl.elem.attr('name')]);

    replicate_https_field();

  };

  embedHandler.setWidthParameter = function(newWidth){

    var newValue = this.elem.val();

    // get position of width parameter

    var startIndex = newValue.indexOf('width=') + 'width='.length;

    // get position of end of width parameter

    var endIndex = newValue.indexOf(' ', startIndex);

    newValue = newValue.substr(0, startIndex) + newWidth + newValue.substr(endIndex);
  ...
  $('.js_embed_order, .js_embed_lang').siblings('label').click(function(){$(this).siblings('input').trigger('click').change(); });

  $('.js_embed_category_select').inputControl({onCleaned: function(){ 

    replicate_select_pick('.js_embed_category_select', this.cleanValues[this.elem.attr('name')]);
    
    set_embed_parameter(this);

  }});
  
  $('.js_embed_location_select').inputControl({onCleaned: function(){

    replicate_select_pick('.js_embed_location_select', this.cleanValues[this.elem.attr('name')]);

    set_embed_parameter(this); 

  }});
  
  $('.js_embed_custom_css').inputControl({onCleaned: function(){ 

    var value = this.cleanValues['css'];
    $('.js_embed_custom_css').each(function(index, item) {
      $(item).val(value);
    });
  ...
  $('.js_embed_order').click(function(e){ $(this).siblings('.js_embed_order').attr('selected',''); }).inputControl({
    onCleaned: function(){ set_embed_parameter(this); }
  });

  $('.js_embed_passed').inputControl({onCleaned: function(){

    embedHandler.setParameter('passed', this.cleanValues['passed']?'0':'');
    facebookHandler.setParameter('passed', this.cleanValues['passed']?'0':'');
    replicate_https_field();

  }});

  $('.js_date_embed_filter').inputControl({
    clean: function(){
    ...
    onCleaned: function(){
      ...
      embedHandler.setParameter('from', this.cleanValues['from']);

      embedHandler.setParameter('to', this.cleanValues['to']);

      facebookHandler.setParameter('from', this.cleanValues['from']);

      facebookHandler.setParameter('to', this.cleanValues['to']);

      replicate_https_field();

      var value = this.cleanValues['from'].length?this.cleanValues['from'] + ' - ' + this.cleanValues['to']:'';
      $('.js_date_embed_filter').each(function(index, item) {
        $(item).val(value);
      });
  ...
  $('.js_embed_lang').click(function(e){ 
    var checked = $(this).val();
    $('.js_embed_lang').each(function(index,item){
      $(item).attr('checked', checked==$(item).val()?'checked':'');
    });
    $(this).siblings('.js_embed_lang').attr('selected',''); 
  }).inputControl({
    onCleaned: function(){

      embedHandler.setLangParameter(this.cleanValues['lang']);
      facebookHandler.setLangParameter(this.cleanValues['lang']);
      replicate_https_field();
  ...
  $('.js_embed_width').inputControl({
    ...
    onCleaned: function(){
      ...
      embedHandler.setWidthParameter(this.cleanValues['width']);
  ...
  $('.js_app_id').inputControl({
    onValidChange: function(value){

      $('.js_facebook_add_page').attr('href', 'https://www.facebook.com/dialog/pagetab?app_id=APPID&next=PROGRAMURL'.replace('APPID', value).replace('PROGRAMURL', $('.js_facebook_code').val())).removeClass('disabled');
