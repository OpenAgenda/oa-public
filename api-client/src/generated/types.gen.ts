...
export type FacetResults = {
    /**
     * One entry per requested facet, under the facet's name. A facet that was not requested is absent; a requested one is always present, even with nothing to report — empty as `[]`, `{}` or `null`, according to its own shape. Shapes differ by facet: the property below for each is the authority, and the `facets` parameter describes them family by family.
     *
     */
    facets: {
