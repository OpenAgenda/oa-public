...
import legacyEventSvc from '../services/event/index.js';
...
const getEventLabel = makeLabelGetter(eventShowLabels);
const getLabel = makeLabelGetter(agendaShowLabels);
...
function showXhr(template) {
  return (req, res, next) => {
    ...
    cmn.renderTemplate(req, template, req.templateData, (err, partial) => {
...
function _formatEventItem(event, req, cb) {
  ...
  const formatted = Object.assign(inst, {
    ...
    freeText: getLongDescriptionHTML(
      ...
      inst.getFreeText() ?? inst.longDescription,
...
function _loadAgendaByAgendaId(req, res, next) {
  ...
  agendaSvc.get({ id: req.agendaId }, (err, agenda) => {
