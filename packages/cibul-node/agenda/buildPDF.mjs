import pdf from '@openagenda/pdf';
import logs from '@openagenda/logs';
import convertEventToLegacyFormat from '@openagenda/legacy/convertEventToLegacyFormat/index.js';

const log = logs('agenda/buildPDF');

export default async function buildPDF(req, res, _next) {
  const {
  ...
  const stream = await core.agendas(req.params.uid).events.search(req.query, null, { stream: true, detailed: true });
