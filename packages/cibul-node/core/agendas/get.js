...
async function get(core, agendaUid, options = {}) {
  ...
  const {
    ...
    includeSplittedMemberSchema = false,
    actingMember = null,
  } = options;
  ...
  if (includeMemberSchema) {
    related.memberSchema = includeSplittedMemberSchema ? await getMemberSchema(services, agenda, { access, actingMember }) : (await getMemberSchema(services, agenda, { access, actingMember })).merged;
  }
