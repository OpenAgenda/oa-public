...
export default async (core, agendaUid, eventUid, query = {}, options = {}) => {
  const { agendaEvents, agendas, members } = core.services;

  const { excludeCurrentAgenda = true } = query;
  ...
  const { items: references } = await agendaEvents.list.byEventUid(
    eventUid,
    {
      state: 2,
      private: false,
      excludeAgendaUid: excludeCurrentAgenda ? agendaUid : null,
    },
    0,
    10,
  );
