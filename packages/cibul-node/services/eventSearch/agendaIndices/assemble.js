...
async function list( agendaEvents, formSchemaId = null, customValidators = null ) {
  ...
  const events = await eventSvc.list( { uid: eventUids, private: null }, 0, eventUids.length, { detailed: true, html: true } ).then( r => r.events );
  ...
  const assembled = agendaEvents.map( ae => {
    ...
    .map( ae => {
      ...
      let custom = _.find( customMap, c => c.identifier === ae.eventUid );
