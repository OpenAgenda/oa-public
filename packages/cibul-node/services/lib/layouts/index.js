    ...
    filenames
      ...
      .map(async (layoutName) => {
        const layoutModule = await import(
          new URL(`./${layoutName}/index.js`, import.meta.url).pathname
        );
        return {
          layoutName,
          layoutModule: layoutModule.default || layoutModule,
        };
      }),
  );

  return layoutModules.reduce(
    (layouts, { layoutName, layoutModule }) =>
      _.set(layouts, layoutName, layoutModule),
    {},
  );
}
...
const mappedLayouts = _.mapValues(
  layouts,
  ({ parent, parser, render }) =>
    (content, data = {}) => {
...
export const load = (layoutName, preLoaded = {}) =>
  (content, data = {}) =>
    mappedLayouts[layoutName](content, { ...preLoaded, ...data });
