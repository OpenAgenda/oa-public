...
export default async ({ services }, ae, context) => {
  ...
  try {
    ...
    if (err) {
      log(
        'error',
        'Error to add activity event.delete, agenda.removeEvent, agenda.removeDeletedEvent or agenda.systemRemoveEvent in feed event:%s',
        event.uid,
        err,
      );
    }
