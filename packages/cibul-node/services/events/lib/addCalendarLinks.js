...
export default function addCalendarLinks(
  { root },
  event,
  eventUrl,
  agenda,
  ...
  event.timings.forEach((timing, i) => {
    ...
    timing.calendarLinks = {
      google: [
        ...
        '&dates=',
        _linkifyTime(timing.begin),
        '/',
        _linkifyTime(timing.end),
        '&sprop=website:',
        eventUrl,
        '&details=',
        encodeURIComponent(`${eventDescription} - ${eventUrl}`),
      ]
        .concat(
          event.location
            ? [
      ...
      yahoo: [
        ...
        _linkifyTime(timing.begin),
        '&DUR=',
        (timing.end - timing.begin) / (1000 * 60 * 60),
        '&DESC=',
        encodeURIComponent(`${eventDescription} - ${eventUrl}`),
        '&URL=',
        eventUrl,
      ...
      live: [
        ...
        encodeURIComponent(eventTitle),
        '&startdt=',
        moment.tz(timing.begin, event.timezone).locale(lang).utc().format(),
        '&enddt=',
        moment.tz(timing.end, event.timezone).locale(lang).utc().format(),
        '&body=',
        encodeURIComponent(`${eventDescription} - ${eventUrl}`),
      ]
        .concat(
