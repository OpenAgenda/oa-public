...
export default (services) => async (locationUid) => {
  ...
  const eventUids = await services.events
    .list(
      {
        locationUid,
      },
  ...
  const agendaUids = await services.agendaEvents.list
    .byEventUid(eventUids)
    .then((result) => result.items.map((ae) => ae.agendaUid));

  const agendaInfos = await services.agendas
    .list({ uid: agendaUids })
    .then((result) =>
      result.agendas.map((a) => ({ uid: a.uid, title: a.title })));
