...
const log = logs('services/agendaLocations/getEventCounts');

export default (config, services) =>
  async (locationUids, { agendaUid }) => {
    log('getting for %s for agenda %s', locationUids.join(', '), agendaUid);
    if (!locationUids.length) return [];
    const { core, events } = services;

    const absoluteCounts = await events.countByLocationUids({
      locationUid: locationUids,
    });

    const {
      aggregations: { locations: agendaEventCounts },
    ...
    return absoluteCounts.reduce(
      ...
      agendaEventCounts.map((e) => ({
        uid: e.key,
        eventCount: 0,
        agendaEventCount: e.eventCount,
      })),
    );
  };
