...
export default (services) =>
  async (location, options = {}) => {
    ...
    if (removeEvents) {
      log(
        'info',
        'deleting events associated with location of uid %s',
        location.uid,
      );
      ...
      do {
        ...
        try {
          ...
          await core
            .agendas(event.agendaUid)
            .events.remove(event.uid, { access: 'internal' });
        } catch (e) {
          offsetErrored += 1;
          log(
            'error',
            'failed to remove event %s with location uid %s',
            event.uid,
            location.uid,
            e,
          );
