...
The upcomingOnly option is by default true and indicates that the events to be added to the aggregator are only the ones that are upcoming. The method queues that operation, and when processed, events of the source agenda are streamed to be published through _evaluatePublish_.
