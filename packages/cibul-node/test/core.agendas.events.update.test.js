...
describe('core - functional (server): core.agendas().events.update()', () => {
  ...
  describe('form schema - contributor write restrictions', () => {
    describe('core', () => {
      it('contributor can update contributor-restricted field (contributorOnlyField)', async () => {
        const event = await core.agendas(55555555).events.patch(
          77777777,
          {
            contributorOnlyField: 'updated by contributor',
          },
          {
            userUid: 24372732, // chrissie - contributor
          },
        );

        expect(event.contributorOnlyField).toBe('updated by contributor');
      });

      it('administrator cannot update contributor-restricted field - field is filtered out', async () => {
        await core.agendas(55555555).events.patch(
          77777778,
          {
            contributorOnlyField: 'set by contributor',
          },
          {
            userUid: 24372732, // chrissie - contributor
          },
        ...
        const event = await core.agendas(55555555).events.patch(
          77777778,
          {
            contributorOnlyField: 'attempted by admin',
            title: { fr: 'Titre modifié par admin' },
          },
          {
            userUid: 10866730, // helene - administrator
          },
        );

        expect(event.contributorOnlyField).toBe('set by contributor');
        expect(event.title.fr).toBe('Titre modifié par admin');
      });

      it('moderator cannot update contributor-restricted field - field is filtered out', async () => {
        const event = await core.agendas(55555555).events.patch(
          77777778,
          {
        ...
        expect(event.contributorOnlyField).toBe('set by contributor');
        expect(event.title.fr).toBe('Titre modifié par moderator');
      });

      it('moderator patching ONLY contributor-restricted field should throw error', async () => {
        let error;
        try {
          await core.agendas(55555555).events.patch(
            77777777,
            {
              contributorOnlyField: 'only restricted field',
            },
            {
              userUid: 82253124, // moderator - has no write access to this field
            },
          );
        } catch (e) {
          error = e;
      ...
      it('administrator patching ONLY contributor-restricted field should throw error', async () => {
        let error;
        try {
          await core.agendas(55555555).events.patch(
            77777777,
            ...
            {
              userUid: 10866730, // administrator - has no write access to this field
            },
          );
        } catch (e) {
          error = e;
        ...
        // Should throw an error because all fields were filtered out
        expect(error).toBeDefined();
        expect(error.message).toBe(
          'All provided fields were filtered due to insufficient permissions',
        );
