...
export default function insertEventSet(knex, raw, eventSetId, extendWith = {}) {
  const { legacyEvent, eventLocation, occurrences, event, agendaEvents } = JSON.parse(
  ...
  if (legacyEvent) {
    raw.push(
      knex('event').insert([
        extendWith.legacyEvent
          ? ih(legacyEvent, extendWith.legacyEvent)
          : legacyEvent,
      ]),
    );
  }
  if (eventLocation) {
    raw.push(
      knex('event_location').insert([
        extendWith.eventLocation
          ? ih(eventLocation, extendWith.eventLocation)
          : eventLocation,
      ]),
    );
  }
  if (occurrences) {
    raw.push(
      knex('occurrence').insert(
        extendWith.occurrences
          ? ih(occurrences, extendWith.occurrences)
          : occurrences,
      ),
    );
  }
