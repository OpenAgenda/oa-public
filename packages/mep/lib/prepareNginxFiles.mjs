...
const renderNginxConf = _.template(
  await fs.promises.readFile(`${__dirname}/../nginx/nginx.conf.tpl`, 'utf-8'),
);
...
function getNodesEndpoints(nodes, groupDisplayName, serverPort) {
  return nodes
    .byGroups([groupDisplayName])
    .map((node) => `${node.address}:${serverPort}`);
}

export default async function prepareNginxFiles({ dir, nodes, envVars }) {
  const {
  ...
  await fs.promises.writeFile(
    ...
    renderNginxConf({
      ...
      internalServerPort,
      APIEndpoints: getNodesEndpoints(nodes, 'api', serverPort),
      NextEndpoints: getNodesEndpoints(nodes, 'next', serverPort),
      WebEndpoints: getNodesEndpoints(nodes, 'web', serverPort),
    }),
    'utf-8',
  );
