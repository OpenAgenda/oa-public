...
describe('event-search - unit: utils - defineIncludes', () => {
  ...
  const { formSchema } = fxWithFormSchemaDetailed;
  ...
  it('detailed returns fields specified in detailedSearchIncludes', () => {
    ...
    expect(
      detailedSearchIncludes.filter((f) => included.includes(f)).length,
    ).toEqual(detailedSearchIncludes.length);
  });
  ...
  it('if formSchema is given and detailed is true, additionalFields and detailed fields are provided', () => {
    ...
    expect(formSchemaFields.filter((f) => included.includes(f)).length).toEqual(
      formSchemaFields.length,
    );
  });
  ...
  it('if access restrictions are present and access is public, only public fields are included', () => {
    ...
    ['custom_description', 'intermunicipal_interest', 'recurring'].forEach(
      (restrictedField) => {
