...
describe('event-search - unit: utils - defineIncludes', function() {
  ...
  it('detailed returns fields specified in detailedSearchIncludes', () => {
    ...
    detailedSearchIncludes.filter(f => included.includes(f)).length.should.equal(detailedSearchIncludes.length);
  });
  ...
  it('if formSchema is given and detailed is true, additionalFields and detailed fields are provided', () => {
    ...
    detailedSearchIncludes.filter(f => included.includes(f)).length.should.equal(detailedSearchIncludes.length);
    formSchemaFields.filter(f => included.includes(f)).length.should.equal(formSchemaFields.length);
  });
  ...
  it('when fields are listed in requested option, they are used to define base include list', () => {
    const included = defineIncludes({
      baseSearchIncludes,
      detailedSearchIncludes
    }, {
  ...
  it('formSchema fields can be included', () => {
    ...
    included.should.eql(['uid', 'thematiques-bordeaux-metropole']);
  });
