...
describe('06 - event search - functional: update', () => {
  ...
  it('udpate the title of an event', async () => {
    await service('test_index').update(
      { uid: 1 },
      {
        title: {
    ...
    const { events } = await service('test_index').search({ uid: 1 });
  ...
  it('updating the title means change can be searched after update', async () => {
    ...
    const { events, total } = await service('test_index').search({
      search: 'Witness',
    });
  ...
  it('if operation option is indexand document is not already indexed, it is added', async () => {
    const { success } = await service('test_index').update(
      { uid: 12001 },
      {
        title: 'I am a new document that is not yet in the index',
      },
