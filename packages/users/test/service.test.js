...
describe( 'methods', () => {
  ...
  describe( 'get', () => {
    ...
    it( 'get user with includeImagePath', async () => {
      ...
      expect( user.image ).toBe( '//openagendatst.s3.amazonaws.com/review_kaore-olafsson_01.jpg' );
  ...
  describe( 'find', () => {
    ...
    it( 'find with uid query', async () => {
      ...
      expect( total ).toBe( 2 );
      expect( users.map( v => v.uid ) ).toEqual( expect.arrayContaining( [ 27639980, 54505079 ] ) );
    } );
  ...
  describe.skip( 'clearImageProfile', () => {
  ...
  describe( 'create', () => {
    it( 'create a user with an already taken email', async () => {
      await expect(
        service.create( { fullName: 'Jean-Eude', email: 'gaetan@cibul.net', password: 'pa**word' } )
  ...
  describe( 'remove', () => {
    it( 'remove a user', async () => {
      ...
      const removedUser = await service.get( 17133001, { removed: null, detailed: true, internal: true } );
  ...
  describe( 'requestChangeEmail', () => {
    it( 'basic requestChangeEmail', async () => {
      await service.requestChangeEmail( kaoreUid, { newEmail: 'jean-meaurice@hotmail.fr' } );
  ...
  describe( 'changePassword', () => {
    it( 'change password', async () => {
      ...
      const result = await knex( config.schemas.user ).select().first().where( { uid: 17133001 } );
  ...
  describe( 'generateApiKey', () => {
    it( 'generate new api public key', async () => {
      const before = await service.get( 17133001 );
  ...
  describe( 'refresh', () => {
    ...
    it( 'refresh lastSignin', async () => {
      const user = await service.refresh( 17133001, {
        lastSignin: true
      }, {
        detailed: true
      } );
