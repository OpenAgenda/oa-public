"use strict";

module.exports = {
  aggregatorExplanation: {
    fr: 'Cette fonction permet de récupérer automatiquement les événements des agendas que vous aurez choisi.',
    en: 'This feature allows you to automatically fetch the events added to the agendas of your choice.'
  },
  createAggregator: {
    fr: 'Activer l\'agrégation d\'agendas',
    en: 'Activate agenda aggregation'
  },
  sourceAgendas: {
    fr: 'Agendas sources',
    en: 'Source agendas'
  },
  sourcesExplanation: {
    fr: 'Les événements publiés par les agendas listés ici sont automatiquement ajoutés à l\'agenda %title%.',
    en: 'The events published by the agendas listed here are automatically added to the %title% agenda.'
  },
  addSources: {
    fr: 'Pour ajouter des sources, <a href="%searchLink%">cherchez les agendas</a> que vous voulez' +
    ' utiliser, ouvrez-les et cliquez sur le lien <strong>utiliser comme source</strong>.',
    en: 'To add sources, <a href="%searchLink%">find the agendas</a> you want to use,' +
    ' open them, and click the <strong>use as source</strong> link.'
  },
  aggregationLoop: {
    fr: 'Cet agenda agrège des événements de l\'agenda auquel vous voulez l\'ajouter. Les boucles d\'agrégation ne sont pas permises.',
    en: 'This agenda agregates events from the agenda you want to add it to. Agregation loops are not permitted.'
  },
  searchAgenda: {
    fr: 'Rechercher un agenda',
    en: 'Search an agenda'
  },
  officialAgenda: {
    fr: 'Agenda officiel',
    en: 'Official agenda'
  },
  noResult: {
    fr: 'Aucun résultat trouvé',
    en: 'No result found'
  },
  removeSource: {
    fr: 'Retirer des sources',
    en: 'Remove source'
  },
  removeConfirmMessage: {
    fr: 'Êtes-vous sûr de vouloir supprimer cet agenda des sources ?',
    en: 'Are you sure you want to delete this agenda from sources ?'
  },
  sourceAdded: {
    en: '%source% was added to the sources of %agg%. Its upcoming events will be added shortly.',
    fr: '%source% a été ajouté aux sources de %agg%. Les événements à venir seront rajoutés dans un instant.'
  },
  sourceRemoved: {
    en: '%source% was removed from the sources of %agg%.',
    fr: '%source% a été retiré des sources de %agg%.'
  },
  totalSources: {
    fr: 'Total des sources',
    en: 'Total sources'
  }
};
