"use strict";

module.exports = {
  "activationRequest": {
    "fr": "Demande d'activation",
    "en": "Activation request",
    "de": "Aktivierungsanforderung",
    "es": "Solicitud de activación",
    "it": "Richiesta di attivazione",
    "io": "crwdns13168:0crwdne13168:0"
  },
  "yourMessage": {
    "fr": "Votre message",
    "en": "Your message",
    "de": "Deine Nachricht",
    "es": "vuestro mensaje",
    "it": "Il vostro messaggio",
    "io": "crwdns13170:0crwdne13170:0"
  },
  "send": {
    "fr": "Envoyer",
    "en": "Send",
    "de": "Senden",
    "es": "enviar a",
    "it": "Inviare a",
    "io": "crwdns13172:0crwdne13172:0"
  },
  "confirmationMessage": {
    "fr": "Message de confirmation",
    "en": "Confirmation message",
    "de": "Bestätigungsmeldung",
    "es": "mensaje de confirmación",
    "it": "Messaggio di conferma",
    "io": "crwdns13174:0crwdne13174:0"
  },
  "requestSuccess": {
    "fr": "Votre demande a bien été envoyée.",
    "en": "Your request has been sent.",
    "de": "Ihre Anfrage wurde verschickt.",
    "es": "Su solicitud ha sido enviada.",
    "it": "La vostra richiesta è stata inviata.",
    "io": "crwdns13176:0crwdne13176:0"
  },
  "requestFail": {
    "fr": "Une erreur a eu lieu lors de l'envoi de votre demande, veuillez réessayer ultérieurement.",
    "en": "An error occurred while sending your request, please try again later.",
    "de": "Fehler beim Absenden deiner Anfrage wieder versuche es später.",
    "es": "Se produjo un error al enviar su solicitud, por favor intente de nuevo más tarde.",
    "it": "Si è verificato un errore durante l'invio della richiesta, si prega di riprovare più tardi.",
    "io": "crwdns13178:0crwdne13178:0"
  },
  "requestTitleInvitationMessage": {
    "fr": "Demande d'activation des messages personnalisés",
    "en": "Request to activate personalized messages",
    "de": "Anfrage zu aktivieren personalisierte Nachrichten",
    "es": "Solicitud de activación de mensajes personalizados",
    "it": "Richiesta di attivazione di messaggi personalizzati",
    "io": "crwdns13180:0crwdne13180:0"
  },
  "requestMessageInvitationMessage": {
    "fr": "Cette fonctionnalité vous permet d'inviter un/des utilisateur(s) avec un message personnalisé, ainsi qu'écrire un message à tous les membres.",
    "en": "This feature allows you to invite a user(s) with a custom message, and writing a message to all members.",
    "de": "Mit dieser Funktion könne einen Benutzer (n) mit einer benutzerdefinierten Nachricht laden, und eine Nachricht an alle Mitglieder zu schreiben.",
    "es": "Esta característica le permite invitar a un / usuario (s) con un mensaje personalizado y escribir un mensaje a todos los miembros.",
    "it": "Questa funzione permette di invitare l'utente o gli utenti con un messaggio personalizzato, così come di scrivere un messaggio a tutti i membri.",
    "io": "crwdns13182:0crwdne13182:0"
  },
  "requestTitleOfficialAgenda": {
    "fr": "Demande d'activation d'un agenda officiel",
    "en": "Request to activate an official agenda",
    "de": "Fordern Sie eine offizielle Kalender zu aktivieren",
    "es": "solicitud de activación de una agenda oficial",
    "it": "Richiesta di attivazione di un agenda ufficiale",
    "io": "crwdns13184:0crwdne13184:0"
  },
  "requestMessageOfficialAgenda": {
    "fr": "Merci de publier au moins un événement sur votre agenda avant votre demande d'officialisation.",
    "en": "Please post at least one event on your agenda before your request for officialization.",
    "de": "Bitte sende mindestens eine Veranstaltung auf der Kalender, bevor Sie Deine Anfrage für officialization.",
    "es": "Gracias a publicar al menos un evento en su agenda antes de que su solicitud de formalización.",
    "it": "Si prega di pubblicare almeno un evento in agenda prima della richiesta di ufficializzazione.",
    "io": "crwdns13186:0crwdne13186:0"
  },
  "requestTitleAgendaSchema": {
    "fr": "Demande de champs personnalisés additionnels",
    "en": "Request for additional custom fields",
    "de": "Antrag auf zusätzliche benutzerdefinierte Felder",
    "es": "Solicitud de campos personalizados adicionales",
    "it": "Richiesta di campi aggiuntivi personalizzati",
    "io": "crwdns13188:0crwdne13188:0"
  },
  "requestMessageAgendaSchema": {
    "fr": "Vous voulez ajouter plus de champs à votre formulaire, en adapter l'ordre ou les instructions? Ecrivez nous un rapide message",
    "en": "You need to add multiple fields to your form,, adapt their order or labels? Send us a short message",
    "de": "Mehrere Felder zum Formular hinzufügen ? Reihenfolge oder Etiketten ändern? Sende uns eine kurze Nachricht",
    "es": "Si desea añadir más campos al formulario, o ajustar el orden de las instrucciones? Escríbanos un mensaje rápido",
    "it": "Volete aggiungere altri campi al vostro modulo, adattare l'ordine o le istruzioni? Scrivici un messaggio veloce",
    "io": "crwdns13190:0crwdne13190:0"
  },
  "requestTitlePrivateAgenda": {
    "fr": "Demande d'activation d'un agenda privé",
    "en": "Request to activate a private agenda",
    "de": "Ein private Veranstaltungskalender aktivieren",
    "es": "solicitud de activación de un agenda íntimo",
    "it": "Richiesta di attivazione di un agenda privato",
    "io": "crwdns13192:0crwdne13192:0"
  },
  "requestMessagePrivateAgenda": {
    "fr": "Cette fonction est nouvelle, expliquez-nous votre usage:",
    "en": "This is a new feature, let us know more about your requirement:",
    "de": "Dies ist eine neue Funktion, lasse uns mehr über Deine Anforderung wissen:",
    "es": "Esta característica es nueva, nos dice su uso:",
    "it": "Questa è una nuova funzione, spiegateci il vostro utilizzo:",
    "io": "crwdns13194:0crwdne13194:0"
  },
  "requestTitleCustomFields": {
    "fr": "Demande d'activation des champs personnalisés",
    "en": "Request to activate custom fields",
    "de": "Fordern Sie benutzerdefinierte Felder aktivieren",
    "es": "campos personalizados de solicitud de activación",
    "it": "Richiesta di attivazione dei campi personalizzati",
    "io": "crwdns13196:0crwdne13196:0"
  },
  "requestMessageCustomFields": {
    "fr": "Vous avez besoin d'ajouter des champs additionnels à votre formulaire de saisie d'événements?\n\nOu encore de lier certains événements de votre agenda entre eux?\n\nDétaillez-nous votre besoin et nous traiterons votre demande dans les plus brefs délais !",
    "en": "Do you need to add custom fields to the event form of your agenda?\n\nOr to link some events of your agenda with each other?\n\nGive us details on your requirements and we'll get back to you shortly!",
    "de": "Benötige benutzerdefinierte Felder auf das Veranstaltung Form deinerVerwaltungskalender zu nehmen?\n\nOder einige Veranstaltungen deinerVerwaltungskalender miteinander zu verknüpfen?\n\nGebe uns Details über Deine Anforderungen und werden uns in Kürze bei Ihnen melden!",
    "es": "Es necesario añadir campos adicionales a su formulario de entrada de evento?\n\nO para unir algunos eventos de agenda entre ellos?\n\nExplicar sus necesidades y vamos a procesar su solicitud tan pronto como sea posible!",
    "it": "Hai bisogno di aggiungere altri campi al tuo modulo di iscrizione all'evento?  O per collegare tra loro alcuni eventi in agenda?  Diteci di cosa avete bisogno e noi elaboreremo la vostra richiesta il prima possibile!",
    "io": "crwdns13198:0crwdne13198:0"
  },
  "requestTitleAdvancedEmbed": {
    "fr": "Activer la personnalisation avancée des gabarits",
    "en": "Activate the advanced template customization",
    "de": "Aktiviere die erweiterte Vorlagenanpassung",
    "es": "Habilitar plantillas de personalización avanzada",
    "it": "Attivare la personalizzazione avanzata dei modelli",
    "io": "crwdns13200:0crwdne13200:0"
  },
  "requestMessageAdvancedEmbed": {
    "fr": "Vous voulez un contrôle complet sur la présentation de votre agenda intégré?\n\nLa personnalisation des gabarits vous permettra de définir vous-même les éléments à afficher ainsi que leurs agencement.\n\nNous pouvons également vous proposer une prestation d'intégration de votre agenda en marque blanche sur votre site\n\nDétaillez-nous votre besoin et nous traiterons votre demande dans les plus brefs délais !",
    "en": "Do you need complete control over the presentation of your integrated agenda?\n\nTemplate customization will give you the tools to define precisely what to display and where to display it.\n\nOur team can also offer to integrate your agenda according to your requirements.\n\nGive us details on your requirements and we'll get back to you shortly!",
    "de": "Habe die vollständige Kontrolle über die Präsentation deiner integriertenVerwaltungskalender benötigen?\n\nVorlagenanpassung finde die Werkzeuge, um zu definieren, genau das, was angezeigt werden und wo es angezeigt werden soll.\n\nUnser Team kann auch DeineVerwaltungskalender zu integrieren nach deinen Anforderungen bieten.\n\nGebe uns Details über Deine Anforderungen und werden uns in Kürze bei Ihnen melden!",
    "es": "Desea tener un control total sobre la presentación de su agenda integrado?\n\npersonalización de plantillas le permiten definir a sí mismo lo que desea mostrar y su disposición.\n\nTambién podemos ofrecer una integración beneficiar a su agenda de marca blanca en su sitio\n\nExplicar sus necesidades y vamos a procesar su solicitud tan pronto como sea posible!",
    "it": "Volete il controllo completo del layout del vostro agenda integrato?  La personalizzazione dei modelli vi permetterà di definire voi stessi gli elementi da visualizzare e il loro layout.  Possiamo anche offrirvi un servizio per integrare la vostra agenda di white-label sul vostro sito. Diteci di cosa avete bisogno e noi elaboreremo la vostra richiesta il prima possibile!",
    "io": "crwdns13202:0crwdne13202:0"
  },
  "requestTitlePublicAgenda": {
    "fr": "Demande d'activation d'un agenda public",
    "en": "Request to activate a public agenda",
    "de": "Fordern Sie eine öffentlicheVerwaltungskalender zu aktivieren",
    "es": "solicitud de activación de un agenda público",
    "it": "Richiesta di attivazione di un agenda pubblico",
    "io": "crwdns13204:0crwdne13204:0"
  },
  "requestTitleLimitDates": {
    "fr": "Bornez les dates des événements contribués",
    "en": "Add limits to the dates of contributed events",
    "de": "In Grenzen zu den Terminen von beigetragen Veranstaltungen",
    "es": "Confinar las fechas de los eventos contribuido",
    "it": "Segnare le date degli eventi contributivi",
    "io": "crwdns13206:0crwdne13206:0"
  },
  "requestMessageLimitDates": {
    "fr": "Bornez le sélecteur de dates du formulaire de saisie de votre agenda.\n\nInteressé(e) ? Laissez nous un message et nous traiterons votre demande dans les plus brefs délais !",
    "en": "Limit contributions to predefined event dates. \n\nInterested by this feature? Give us details on your requirements and we'll get back to you shortly!",
    "de": "Begrenze Beiträge zu vordefinierten Veranstaltungdaten.\n\nGünstiger dieses Feature? Gebe uns Details über Deine Anforderungen und werden uns in Kürze bei Ihnen melden!",
    "es": "Confinar el selector de fecha de su formulario de entrada de agenda.\n\nInteresado (a)? nos deja un mensaje y vamos a procesar su solicitud tan pronto como sea posible!",
    "it": "Legare il selettore di data del modulo di iscrizione della data dell'ordine del giorno.  Sei interessato? Lasciateci un messaggio e provvederemo ad elaborare la vostra richiesta il prima possibile!",
    "io": "crwdns13208:0crwdne13208:0"
  },
  "requestTitleCustomMessage": {
    "fr": "Personnalisation des messages",
    "en": "Customization of messages",
    "de": "Anpassung von Meldungen",
    "es": "personalizar los mensajes",
    "it": "Personalizzazione dei messaggi",
    "io": "crwdns13210:0crwdne13210:0"
  },
  "requestMessageCustomMessage": {
    "fr": "Cette fonction vous permet de modifier les messages envoyés à vos membres lors d'une invitation à contribution ou bien une publication d'événement.\n\nInterressé(e) ? Laissez nous un message et nous traiterons votre demande dans les plus brefs délais !",
    "en": "This feature allows you to customize messages sent to contributors when they are invited to contribute or when their event is published.\n\nGive us details on your requirements and we'll get back to you shortly!",
    "de": "Mit dieser Funktion könne Nachrichten an die Mitglieder geschickt anpassen, wenn sie eingeladen sind, dazu beitragen, oder wenn Deine Veranstaltung veröffentlicht.\n\nGebe uns Details über Deine Anforderungen und werden uns in Kürze bei Ihnen melden!",
    "es": "Esta función le permite editar los mensajes enviados a sus miembros cuando una invitación a una contribución o la publicación de sucesos.\n\nInteresado (a)? nos deja un mensaje y vamos a procesar su solicitud tan pronto como sea posible!",
    "it": "Questa funzione vi permette di modificare i messaggi inviati ai vostri soci quando viene inviato un invito a contribuire o la pubblicazione di un evento.  Sei interessato? Lasciateci un messaggio e provvederemo ad elaborare la vostra richiesta il prima possibile!",
    "io": "crwdns13212:0crwdne13212:0"
  },
  "requestTitleModerators": {
    "fr": "Membres modérateurs",
    "en": "Moderator members",
    "de": "Moderator Mitglieder",
    "es": "miembros moderadores",
    "it": "Moderatori membri",
    "io": "crwdns13214:0crwdne13214:0"
  },
  "requestMessageModerators": {
    "fr": "Les modérateurs sont des membres pouvant éditer tous les événements d'un agenda, sans pour autant avoir les accès de paramétrages.\n\nDétaillez-nous votre besoin et nous traiterons votre demande dans les plus brefs délais !",
    "en": "Moderators are members that can edit the events of an agenda and change event states, without having access to agenda settings.\n\nGive us details on your requirements and we'll get back to you shortly!",
    "de": "Moderatoren sind Mitglieder, die die Veranstaltungen einerVerwaltungskalender bearbeiten und Veranstaltungzustände ändern, ohne Einstellungen Zugriff aufVerwaltungskalender zu haben.\n\nGebe uns Details über Deine Anforderungen und werden uns in Kürze bei Ihnen melden!",
    "es": "Los moderadores son miembros que pueden editar todas agenda de eventos, sin tener la configuración de acceso.\n\nExplicar sus necesidades y vamos a procesar su solicitud tan pronto como sea posible!",
    "it": "I moderatori sono membri che possono modificare tutti gli eventi di un calendario, senza avere accesso alle impostazioni.  Diteci di cosa avete bisogno e noi elaboreremo la vostra richiesta il prima possibile!",
    "io": "crwdns13216:0crwdne13216:0"
  },
  "requestTitleWriteToAll": {
    "fr": "Demande d'activation de la fonction \"Leur écrire\"",
    "en": "Request to activate the \"Write to them\" function",
    "de": "Anfrage zur Aktivierung des „Schreibe sie“ -Funktion",
    "es": "Solicitud de activación de la función \"Su escritura\"",
    "it": "Richiesta di attivazione della funzione \"Scrivi a loro\".",
    "io": "crwdns13218:0crwdne13218:0"
  },
  "requestMessageWriteToAll": {
    "fr": "Envoyez un message à tous les membres de votre agenda ou envoyez une relance personnalisée à tous les membres n'ayant pas encore contribué.\n\nInterressé(e) ? Laissez nous un message et nous traiterons votre demande dans les plus brefs délais !",
    "en": "Send a message to all members of your agenda or target the ones that have not yet contributed.\n\nGive us details on your requirements and we'll get back to you shortly!",
    "de": "Schicke eine Nachricht an alle Mitglieder deinerVerwaltungskalender oder diejenigen zielen, die noch nicht beigetragen haben.\n\nGebe uns Details über Deine Anforderungen und werden uns in Kürze bei Ihnen melden!",
    "es": "Enviar un mensaje a todos los miembros de su agenda o enviar una recuperación personalizada a todos los miembros que aún no han contribuido.\n\nInteresado (a)? nos deja un mensaje y vamos a procesar su solicitud tan pronto como sea posible!",
    "it": "Invia un messaggio a tutti i membri del tuo agenda o invia un promemoria personalizzato a tutti i membri che non hanno ancora contribuito.  Sei interessato? Lasciateci un messaggio e provvederemo ad elaborare la vostra richiesta il prima possibile!",
    "io": "crwdns13220:0crwdne13220:0"
  },
  "requestTitleAggregator": {
    "fr": "Agrégation automatique d'événements",
    "en": "Automatic event aggregation",
    "de": "Automatische Veranstaltung-Aggregation",
    "es": "La agregación automática de eventos",
    "it": "Aggregazione automatica degli eventi",
    "io": "crwdns13222:0crwdne13222:0"
  },
  "requestMessageAggregator": {
    "fr": "Agrégez automatiquement les événements publiés sur d'autres agendas de la plateforme. Demandez l'activation de la fonction, les 365 premières agrégations sont gratuites",
    "en": "Automatically aggregate events that were published on other agendas. Request the activation of this feature, the first 365 aggregations are free",
    "de": "Automatisch aggregierten Veranstaltungen, die auf anderen Kalenders veröffentlicht wurden. Fordern Sie die Aktivierung dieser Funktion werden die ersten 365 Aggregationen sind frei",
    "es": "eventos agrupados publicados automáticamente en otras agendas de la plataforma. Pedir la activación de la función, la primera 365 son agregaciones libres",
    "it": "Aggrega automaticamente gli eventi pubblicati su altre agende della piattaforma. Richiedere l'attivazione della funzione, le prime 365 aggregazioni sono gratuite.",
    "io": "crwdns13224:0crwdne13224:0"
  }
}
