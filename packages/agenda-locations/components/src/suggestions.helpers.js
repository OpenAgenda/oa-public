...
function getLangAlternatives( fieldName, alternatives ) {
  ...
  return alternatives.reduce( ( langs, alternative ) => {
    ...
    if ( !alternative.location[ fieldName ] || typeof alternative.location[ fieldName ] !== 'object' ) {

      return langs;

    }

    return langs.concat( Object.keys( alternative.location[ fieldName ] ).filter( lang => langs.indexOf( lang ) === -1 ) );
...
function getSameAsSuggestedTagIds( tagSet, location, alternatives ) {
  ...
  let tags = tagSet.groups.reduce( ( tags, group ) => tags.concat( group.tags ), [] );

  return tags.filter( t => !suggestedTagsDiffer( t, location, alternatives ).length )
...
function suggestedTagsDiffer( tag, location, alternatives ) {
  ...
  let isInLocation = !!( location && location.tags || [] ).filter( t => t.id === tag.id ).length,

  differentAlternatives = alternatives

  .filter( a => {

    let alternativeTags = a.location.tags || [],

    isInAlternative = !!alternativeTags.filter( t => t.id === tag.id ).length;
...
function prepareAlternatives( location, props, labels, suggestionIndex ) {
  ...
  fields = Object.keys( location ).filter( f => f !== 'state' ),
  ...
  fields.forEach( f => {
    ...
    // basic text fields
    } else if ( userSuggestion && userSuggestion.location[ f ] !== undefined && typeof userSuggestion.location[ f ] !== 'object' ) {
      ...
      for ( let l in userSuggestion.location[ f ] ) {
