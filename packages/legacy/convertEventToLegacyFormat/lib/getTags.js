...
export default (agendaSettings, event) => {
  ...
  const tagFields = formSchema.fields.filter((f) =>
    Object.keys(event).find(
      (el) => f.field === el && (f.origin === 'tags' || f.origin === null),
    ));

  const tagsList = flattenTagSet(tagSet, tagFields);

  const filteredTags = tagsList.filter((tag) => {
    if (Array.isArray(event[tag.fieldSlug])) {
      return (
        event[tag.fieldSlug] && event[tag.fieldSlug].includes(tag.optionId)
      );
    }
    ...
    ? tagSet.groups
      ...
      .reduce((carry, tagGroup) => {
        const tag = filteredTags
          .filter((filteredTag) => filteredTag.field === tagGroup.name)
          .map((filteredTag) => ({
        ...
        if (tag.length) {
          const item = {
            ...
            slug:
                  tagGroup.slug
                  ?? filteredTags.find(
                    (filteredTag) => filteredTag.field === tagGroup.name,
                  )?.fieldSlug,
            tags: tag,
          };
          carry.push(item);
  ...
  const tags = tagGroups.length
    ? tagGroups.reduce((carry, tagGroup) => carry.concat(tagGroup.tags), [])
    : [];
