...
import setMembers from './utils/setMembers.js';
import setSettings from './utils/setSettings.js';

const log = logs('controlData/rebuild');

export function isRebuilding(redis, prefix, uid) {
  return redis.get(`${prefix + uid}:rebuild`).then((result) => !!result);
}

export default async function rebuild({ prefix, knex, redis }, uid) {
  ...
  while (
    (agendaEvent = await knex('agenda_event')
    ...
    if (!location) {
      log(
        'error',
        'did not find location %s of event %s',
        event.uid,
        event.locationUid,
      );
    ...
    try {
      ...
      count += 1;
      log(
        '%s: inserted event %s to agenda %s',
        count,
  ...
  log(
    'info',
    'rebuild complete for %s with %s events in %s s',
    uid,
    ctl.ev.length,
