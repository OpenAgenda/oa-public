...
export default async ({ knex, imagePath }, embedUid) => {
  ...
  const ebd = {
    ...
    mc: _.toPairs(_.get(store, 'layout.mapCorners', {}))
      .map((p) => p[1])
      .join('|'),
  ...
  if (_.get(store, 'layout.mapIcons')) {
    ebd.mi = {
      a: _.get(store, 'layout.mapIcons.active')
        ? imagePath + _.get(store, 'layout.mapIcons.active')
        : false,
      i: _.get(store, 'layout.mapIcons.inactive')
        ? imagePath + _.get(store, 'layout.mapIcons.inactive')
        : false,
    };
