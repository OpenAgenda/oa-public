...
import { isLegacyCustom } from './utils/generateCustomSet.js';
...
async function _getSchema(knex, formSchemaId) {
  return knex('form_schema')
    ...
    .then((r) => (r ? { id: formSchemaId, ...JSON.parse(r.store) } : null));
}

function _labelizeOptionedCustomValues(mapItem) {
  if (!mapItem.field.options) return mapItem;

  const _labelOf = (value) =>
    _.get(_.find(mapItem.field.options, { id: value }), 'value');

  if (_.isArray(mapItem.value)) {
    return Object.assign(mapItem, {
      value: _.uniq(mapItem.value.map(_labelOf)),
    });
  }
...
async function _setCustomValues(knex, eventId, fieldValueMap) {
  ...
  const legacyCustom = fieldValueMap
    ...
    .reduce(
      (obj, mapItem) => _.set(obj, mapItem.field.field, mapItem.value),
      {},
    );

  const current = JSON.parse(
    _.first(await knex('event').pluck('custom_fields').where('id', eventId))
      || '{}',
  );
...
async function _setCategories(knex, agendaId, reviewArticleId, fieldValueMap) {
  log(
    'setting categories for agenda id %s, raId %s',
    agendaId,
    reviewArticleId,
  );
  ...
  const categoryItem = _.first(
    fieldValueMap.filter(
      (mapItem) =>
        mapItem.field.origin === 'categories'
        && mapItem.field.fieldType !== 'abstract',
    ),
  );

  const label = categoryItem
    ? _optionFlatLabel(
  ...
  const matchingTags = fieldValueMap
    ...
    .map((schemaOptionId) => {
      ...
      if (!tag) {
        log(
          'warn',
          'schema option does not have a matching tag',
          schemaOptionId,
        );
      }
      return tag;
...
export default async function set(
  ...
  log(
    'info',
    'setting tags and custom for agenda id %s, event uid %s',
    agendaId,
    eventUid,
  );
...
export async function loadAndSet({ knex }, agendaId, eventUid) {
  log('processing load');

  const schemas = [];
  const customValues = [];

  const { formSchemaId, networkUid } = await knex('review')
    .first([
      'uid as agendaUid',
