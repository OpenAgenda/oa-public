...
import generateCustomFields from '../lib/utils/generateCustomSet.js';
import maisonDesAines from './fixtures/schemas/maison-des-aines.json' with { type: 'json' };
import reedexpo from './fixtures/schemas/reedexpo.json' with { type: 'json' };
...
function _get(fixtureFile) {
  return JSON.parse(
    fs.readFileSync(`${import.meta.dirname}/fixtures/${fixtureFile}`, 'utf-8'),
  );
}
...
describe('03 - utils - generateCustomSet', () => {
  it('schema to text custom field', () => {
    expect(
      generateCustomFields(_get('schemas/text.json')).customFields,
    ).toEqual(_get('customSets/text.json'));
  });

  it('schema to number custom field', () => {
    expect(
      generateCustomFields(_get('schemas/number.json')).customFields,
    ).toEqual(_get('customSets/number.json'));
  });

  it('schema boolean to checkbox custom field', () => {
    expect(
      generateCustomFields(_get('schemas/boolean.json')).customFields,
    ).toEqual(_get('customSets/checkbox.json'));
  });

  it('schema phone to text custom field', () => {
    expect(
      generateCustomFields(_get('schemas/phone.json')).customFields,
    ).toEqual(_get('customSets/textFromPhone.json'));
  });
      ...
      fields: [
        {
          ...
          label:
            'Vous pouvez également charger ici une présentation de votre événement, elle ne sera visible que des délégations et pas du grand public.',
          origin: 'custom',
        },
      ],
    });
