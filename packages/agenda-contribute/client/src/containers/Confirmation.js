...
export default connect(
  ...
  {config?.confirmation?.message
    ? <Instructions message={config.confirmation.message} className="margin-bottom-lg" />
    : <div className="padding-h-md padding-top-lg padding-bottom-xs wsq">
        <p className="text-center margin-bottom-xs margin-top-sm">{defaultConfirmationMessages[config.confirmation?.state === undefined ? 2 : config.confirmation?.state][config.lang]}</p>
      </div>
  }
