        ...
        queryModifier(query).then((rows) => {
          ...
          async.eachOfSeries(
            ...
            (item, i, ecb) => {
              eachCb(
                item,
                rowsAffected - rows.length + Number.parseInt(i, 10),
                ecb,
              );
            },
            dcb,
...
_traverseTable(
  ...
  (q) =>
    q
      ...
      .join(
        config.schemas.user,
        `${config.schemas.apiKeySet}.user_id`,
        `${config.schemas.user}.id`,
      )
    ...
    return knex.transaction(async (t) => {
      try {
        for (const item of keysToCreate) {
          await knex(config.schemas.key).transacting(t).insert(item);
        }
      } catch (e) {
        console.error(e);
