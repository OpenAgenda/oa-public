...
describe('date-range', () => {
  ...
  describe('one date', () => {
    it('should displays as "Tuesday 18 December 2018, 07:00, 10:00, 11:00"', () => {
      expect(range(testData.oneDate.winterDefault, 'en')).toBe(
        'Tuesday 18 December 2018, 07:00, 10:00, 11:00',
      );
    });

    it('should display as "Tuesday 18 December 2018, 08:00, 11:00, 12:00"', () => {
      expect(range(testData.oneDate.winterDefault, 'en', 'Europe/Paris')).toBe(
        'Tuesday 18 December 2018, 08:00, 11:00, 12:00',
      );
    });

    it('should display as "Wednesday 18 April 2018, 09:00, 12:00, 13:00"', () => {
      expect(range(testData.oneDate.summerDefault, 'en', 'Europe/Paris')).toBe(
        'Wednesday 18 April 2018, 09:00, 12:00, 13:00',
      );
    });

    const output2 = 'Thursday 18 December 2014, 07:00, 10:00, 11:00';

    it(`differentYear should displays as "${output2}"`, () => {
      expect(range(testData.oneDate.differentYear, 'en')).toBe(output2);
    });

    const output3 = 'Jeudi 18 décembre 2014, 07h00, 10h00, 11h00';
  ...
  describe('Case: two dates', () => {
    ...
    it('if one date on different year only, display both years: 16 décembre 2014 et 18 décembre 2016', () => {
      expect(range(testData.twoDates.oneDifferentYear, 'fr')).toBe(
        '16 décembre 2014 et 18 décembre 2018',
      );
    });
  });

  describe('Case: more dates', () => {
    const output = '16 - 19 December 2018';
    ...
    it('should display months when more than one', () => {
      expect(range(testData.moreDates.multipleMonths, 'en')).toBe(
        '16 November - 19 December 2018',
      );
    });
  });
  ...
  describe('wrong input', () => {
    it('renders in english if unknown or non existing language is input', () => {
      expect(range(testData.moreDates.default, 'ak')).toBe(
        '16 - 19 December 2018',
      );
    });
  });
  ...
  describe('patterns', () => {
    it('appends an information relative to the day of the week: 1 - 22 december 2015, every Tuesday', () => {
      expect(range(testData.moreDates.tuesdays, 'en')).toBe(
        '1 - 22 December 2015, every Tuesday',
      );
    });
    it('appends an information relative to the day of the week: 1 - 22 december 2015, les mardis', () => {
      expect(range(testData.moreDates.tuesdays, 'fr')).toBe(
        '1 - 22 décembre 2015, les mardis',
      );
    });
  });
});
