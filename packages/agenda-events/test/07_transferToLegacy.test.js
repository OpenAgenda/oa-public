...
describe('agenda-events - 07 - transferToLegacy - transfer to legacy', () => {
  let svc;
  ...
  describe('creating a legacy record', () => {
    ...
    beforeAll(async () => {
      ...
      beforeCreate = await knexClient('review_article').first('id').where({
        event_id: event.id,
        review_id: agenda.id,
      });
    });
    ...
    beforeAll(async () => {
      afterCreate = await knexClient('review_article').first().where({
        event_id: event.id,
        review_id: agenda.id,
      });
    });
    ...
    it('agenda_event reference stores legacy id', async () => {
      ...
      expect(updatedRef.legacyId).toBe(`${agenda.id}.${event.id}`);
    });
  });

  describe('other', () => {
    it('adds user reference when specified', async () => {
      const { created } = await svc(62792452 /* id: 4608 */).create(
        43393865 /* id: 190093 */,
        { userUid: 98842070 },
      );
      ...
      const after = await knexClient('review_article').first().where({
        event_id: 190093,
        review_id: 4608,
      });
    ...
    it('updates record when existing', async () => {
      ...
      const before = await knexClient('review_article').first().where({
        event_id: 81631,
        review_id: 4608,
      });
    ...
    it('removes record', async () => {
      const before = await knexClient('review_article')
        .first()
      ...
      const after = await knexClient('review_article')
        .first()
        .where({ event_id: 81631, review_id: 4608 });
    ...
    it('adds event_editor entry if edition right is not set', async () => {
      ...
      const after = await knexClient('event_editor').first().where({
        event_id: 190093,
        review_id: 4608,
      });
    ...
    it('removes event_editor entry if edition right is set', async () => {
      const { updated } = await svc(62792452).update(6973928, {
        canEdit: false,
      });
