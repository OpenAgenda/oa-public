...
function follow(config, ...rest) {
  const { service, knex } = config;

  const { identifiers, originFeedId, store, cb } = parseArguments(...rest);
  ...
  const promise = service
    ...
    .then(({ targetFeed, originFeed }) => {
      ...
      return knex(config.schemas.feed_follow)
        ...
        .limit(1)
        .then(([feedFollow]) => {
          if (feedFollow) throw new Error('Feed already followed');
    ...
    .then(({ targetFeed, originFeed }) => {
      ...
      return knex(config.schemas.feed_follow)
        .insert({
          target_feed: targetFeed.id,
          origin_feed: originFeed.id,
