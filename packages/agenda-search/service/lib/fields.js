...
const pathIsInRequestedFields = (fields = [], path = []) =>
  path?.length && fields.includes(path.join('.'));

const addField = (includes, field, { path }) =>
  includes.concat((path || []).concat(field.field).join('.'));
...
function buildPaths(fields, options = {}) {
  ...
  return fields.reduce((paths, field) => {
    ...
    if (field.fields) {
      return paths.concat(
        buildPaths(field.fields, { path: path.concat(field.field) }),
      );
    }
    return paths;
  }, []);
...
function _defineIncludes(fields, options = {}) {
  return fields.reduce((includes, field) => {
    ...
    if (
      ...
      && options.onlyIncludeFields.includes(field.field)
    ) {
      return addField(includes, field, options);
    }
    if (options.onlyIncludeFields?.length) {
      return includes;
    }
    ...
    if (
      ...
      && options.includeFields.includes(field.field)
    ) {
      return addField(includes, field, options);
    }
