...
describe('unit - assigning schema properties to another schema', () => {
  ...
  describe('filter by access', () => {
    ...
    it('if access option is not provided, all fields are returned', () => {
      expect(
        merge(networkSchema, agendaSchema).fields.map((f) => f.field),
  ...
  describe('other', () => {
    ...
    it('order of fields is dictated by outer-most schema', () => {
      ...
      expect(
        merge(eventSchema, networkSchema, agendaSchema).fields.map(
          (f) => f.field,
        ),
      ).toStrictEqual(['title', 'participants', 'description', 'theme']);
    });
    ...
    it('merge extends schemas', () => {
      ...
      expect(merge(s1, s2, s3)).toStrictEqual({
        custom: {},
        fields: [
          {
            field: 'budget',
    ...
    it('merge can render optional field non-optional', () => {
      ...
      expect(
        merge(schema, abstract).fields.filter(
          (f) => f.field === 'imageCredits',
        )[0].optional,
      ).toBeFalsy();
    });
    ...
    it('origin value is maintained when present', () => {
      ...
      expect(merged.fields.map((f) => f.origin)).toStrictEqual([
        'custom',
        'tags',
      ]);
    });
