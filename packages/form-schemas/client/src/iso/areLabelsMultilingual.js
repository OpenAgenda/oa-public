...
module.exports = function areFieldLabelsMultilingual(field) {
  ...
  if (
    definedLabelFields.filter((f) => !isMultilingual(field, f)).length
    !== definedLabelFields.length
  ) {
    return true;
  }

  if (field.options) {
    const multilingualOptions = field.options.filter((o) =>
      areFieldLabelsMultilingual(o));
