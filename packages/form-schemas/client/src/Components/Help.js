...
export default function Help({ lang, label, content, link }) {
  if (content) {
    return (
      <MoreInfo
        content={(
      ...
      >
        <a target={_target(link)} href={link}>
          {label || labels.help[lang]}
        </a>
      </MoreInfo>
    );
  }
  ...
  return (
    <a className="margin-right-xs" target={_target(link)} href={link}>
      {label || labels.help[lang]}
    </a>
  );
}
