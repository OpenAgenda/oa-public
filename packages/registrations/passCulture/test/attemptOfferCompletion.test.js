...
describe('attemptOfferCompletion', () => {
  ...
  beforeAll(async () => {
    ...
    const createEventOfferResp = await createEventOffer(pc, {
      title: { fr: 'DHM' },
      timings: [{
  ...
  it('CompleteIncompleteOffer', async () => {
    const res2 = await attemptOfferCompletion({ pc }, { eventOfferId: eventOffer.id, datesPayload }, { eventUid: 1, agendaUid: 2 }, { simulatePending: false });
    expect(res2).toBe(true);
  });
  it('completion of the offer will call OaPatchEvent to update registration', async () => new Promise(rs => {
    ...
    }, { eventOfferId: eventOffer.id, datesPayload: [] }, { eventUid: 1, agendaUid: 2 }, { simulatePending: false });
  }));
  it('Inactive offer is still updated', async () => {
    const res = await attemptOfferCompletion({ pc }, { eventOfferId: eventOffer.id, datesPayload: [] }, { eventUid: null, agendaUid: null }, { simulatePending: false });
    expect(res).toBe(true);
  });
  it('Pending offer is overlooked', async () => {
    const res = await attemptOfferCompletion({ pc }, { eventOfferId: eventOffer.id, datesPayload: [] }, { eventUid: null, agendaUid: null }, { simulatePending: true });
