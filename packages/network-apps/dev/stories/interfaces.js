...
module.exports = (fixtures) => {
  ...
  async function addAgendaToNetwork(uid) {
    const addedAgenda = {
    ...
    _.assign(
      ...
      ih(
        networks,
        _.set({}, [_.findIndex(networks, { uid }), 'agendas'], {
  ...
  async function removeAgendaFromNetwork(uid, agendaUid) {
    ...
    const removedAgendaIndex = _.findIndex(
      networks[networkIndex].agendas,
      (a) => a.uid === agendaUid,
    );
    const removedAgenda = networks[networkIndex].agendas[removedAgendaIndex];

    networks[networkIndex].agendas = networks[networkIndex].agendas.splice(
      removedAgendaIndex,
      1,
    );
  ...
  async function setNetworkSchemaFields(uid, fields) {
    ...
    _.set(networks, [_.findIndex(networks, { uid }), 'schema'], { fields });
