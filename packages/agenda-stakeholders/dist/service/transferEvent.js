...
var log = require('@openagenda/logs')('transferEvent');
...
module.exports = function (agendaId) {
  ...
  return function (params, cb) {
    ...
    }, params)).then(dbUtils.getEvent('event', 'transferedEvent')).then(dbUtils.getAgendaEvent('agenda', 'event', 'agendaEvent')).then(dbUtils.getStakeholder('agenda', 'user', 'stakeholder')).then(_verifyDbFetches).then(function (v) {

      if (!interfaces || !interfaces.beforeTransferEvent) return v;

      return new Promise(function (resolve, reject) {

        interfaces.beforeTransferEvent(v.transferedEvent.uid, v.transferedEvent.ownerId, v.user.id, function (err) {
    ...
    }).then(dbUtils.updateAgendaEvent('agenda', 'event', 'stakeholder')).then(_updateOwnership).done(function (v) {
...
function _updateOwnership(v) {
  ...
  return knex.transaction(function (trx) {
    ...
    return trx.table(schemas.event).where({
