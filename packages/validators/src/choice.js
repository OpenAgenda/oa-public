...
function preClean(params, value) {
  const optionsAreIntegers =
    params.options.filter((o) => Number.isInteger(o)).length ===
  ...
  return []
    ...
    .map((v) => {
      ...
      return params.options.indexOf(
        optionsAreIntegers ? parseInt(valuePart, 10) : valuePart,
      );
    })
    ...
    (value) => {
      ...
      if (params.min && clean.length < params.min) {
        throw errors(
          ...
          value,
          'choice.required.min',
          'between %min% and %max% choices must be made',
          {
            values: {
              min: params.min,
              max: params.max,
