...
import cleanSchema from './clean.js';
...
function schema(options) {
  ...
  function validate(value) {
    const flattened = schemaUtils.mapValuesToValidators(
      params.fields,
      value,
      defaultValue,
    );
    ...
    flattened.forEach((flat) => {
      try {
        clean[flat.field] = flat.isEnabled
          ? flat.validator(
              flat.value,
              Object.keys(flat.related ?? {}).length
                ? { related: flat.related }
                : {},
            )
          : null;
      } catch (errs) {
  ...
  function part(path, value, contextValues) {
    ...
    if (_.isObject(path)) {
      ...
      return parts(
        _.keys(path).filter((field) => schemaFields.includes(field)),
        path,
      );
    }
    ...
    if (
      cursor.enableWith &&
      !withFieldValueMatches(cursor, 'enableWith', contextValues, params.fields)
    ) {
      return null;
    }
