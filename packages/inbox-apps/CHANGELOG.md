# Change Log

All notable changes to this project will be documented in this file.
See [Conventional Commits](https://conventionalcommits.org) for commit guidelines.

### [1.2.1](https://bitbucket.org/openagenda/oa/branches/compare/@openagenda/inbox-apps@1.2.1..@openagenda/inbox-apps@1.2.0) (2020-11-23)

**Note:** Version bump only for package @openagenda/inbox-apps





## [1.2.0](https://bitbucket.org/openagenda/oa/branches/compare/@openagenda/inbox-apps@1.2.0..@openagenda/inbox-apps@1.1.15) (2020-11-23)


### Features

* remove agendaAdmin titles ([3f964e7](https://bitbucket.org/openagenda/oa/commits/3f964e72137b378b66e8d104b756522264df61e6))
* update moment and moment-timezone ([99dc602](https://bitbucket.org/openagenda/oa/commits/99dc602a8f374a3a2d40c2c7d47908b602dfd878))


### Bug Fixes

* **inbox-apps:** fix upload on agenda inbox ([edea674](https://bitbucket.org/openagenda/oa/commits/edea674ddf8abdced7ae7080b397d996bc7397ac))



### [1.1.15](https://bitbucket.org/openagenda/oa/branches/compare/@openagenda/inbox-apps@1.1.15..@openagenda/inbox-apps@1.1.14) (2020-07-16)

**Note:** Version bump only for package @openagenda/inbox-apps





### [1.1.14](https://bitbucket.org/openagenda/oa/branches/compare/@openagenda/inbox-apps@1.1.14..@openagenda/inbox-apps@1.1.13) (2020-07-13)

**Note:** Version bump only for package @openagenda/inbox-apps





### [1.1.13](https://bitbucket.org/openagenda/oa/branches/compare/@openagenda/inbox-apps@1.1.13..@openagenda/inbox-apps@1.1.12) (2020-07-10)


### Bug Fixes

* fix links in changelogs ([84e2460](https://bitbucket.org/openagenda/oa/commits/84e24609981f4ee3bb9e34ef52109d74abe97a62))



### [1.1.12](https://bitbucket.org/openagenda/oa/branches/compare/@openagenda/inbox-apps@1.1.12..@openagenda/inbox-apps@1.1.11) (2020-07-08)

**Note:** Version bump only for package @openagenda/inbox-apps





### [1.1.11](https://bitbucket.org/openagenda/oa/branches/compare/@openagenda/inbox-apps@1.1.11..@openagenda/inbox-apps@1.1.10) (2020-07-06)

**Note:** Version bump only for package @openagenda/inbox-apps





### [1.1.10](https://bitbucket.org/openagenda/oa/branches/compare/@openagenda/inbox-apps@1.1.10..@openagenda/inbox-apps@1.1.9) (2020-06-18)

**Note:** Version bump only for package @openagenda/inbox-apps





### [1.1.9](https://bitbucket.org/openagenda/oa/branches/compare/@openagenda/inbox-apps@1.1.9..@openagenda/inbox-apps@1.1.8) (2020-06-09)

**Note:** Version bump only for package @openagenda/inbox-apps





### [1.1.8](https://bitbucket.org/openagenda/oa/branches/compare/@openagenda/inbox-apps@1.1.8..@openagenda/inbox-apps@1.1.7) (2020-05-22)

**Note:** Version bump only for package @openagenda/inbox-apps





### [1.1.7](https://bitbucket.org/openagenda/oa/branches/compare/@openagenda/inbox-apps@1.1.7..@openagenda/inbox-apps@1.1.6) (2020-05-19)

**Note:** Version bump only for package @openagenda/inbox-apps





### [1.1.6](https://bitbucket.org/openagenda/oa/branches/compare/@openagenda/inbox-apps@1.1.6..@openagenda/inbox-apps@1.1.5) (2020-05-15)

**Note:** Version bump only for package @openagenda/inbox-apps





### [1.1.5](https://bitbucket.org/openagenda/oa/branches/compare/@openagenda/inbox-apps@1.1.5..@openagenda/inbox-apps@1.1.4) (2020-04-07)

**Note:** Version bump only for package @openagenda/inbox-apps





## <small>1.1.4 (2020-04-03)</small>

* add deutsch language choice ([a7f047d](https://bitbucket.org/openagenda/oa/commits/a7f047d))
* add inbox autofocus ([198e92d](https://bitbucket.org/openagenda/oa/commits/198e92d))
* add inboxes to webapp ([a62b9d8](https://bitbucket.org/openagenda/oa/commits/a62b9d8))
* add member-apps to webapp ([021e9f8](https://bitbucket.org/openagenda/oa/commits/021e9f8))
* add missing wrapApp ([ec46a63](https://bitbucket.org/openagenda/oa/commits/ec46a63))
* add NotFound component ([0f590b1](https://bitbucket.org/openagenda/oa/commits/0f590b1))
* add react-integration-app package ([bab0d17](https://bitbucket.org/openagenda/oa/commits/bab0d17))
* aggregator-sources: refactor ([9a10484](https://bitbucket.org/openagenda/oa/commits/9a10484))
* cibul-node: fix not found ([a8b72bc](https://bitbucket.org/openagenda/oa/commits/a8b72bc))
* fix builds on Yarn 2 ([f4723be](https://bitbucket.org/openagenda/oa/commits/f4723be))
* fix decorators, eslint and HMR ([3eb9b89](https://bitbucket.org/openagenda/oa/commits/3eb9b89))
* fix dependencies ([1b2467e](https://bitbucket.org/openagenda/oa/commits/1b2467e))
* fix dependencies ([ee85d7a](https://bitbucket.org/openagenda/oa/commits/ee85d7a))
* fix deps ([71703f8](https://bitbucket.org/openagenda/oa/commits/71703f8))
* fix inbox-apps ([18032c1](https://bitbucket.org/openagenda/oa/commits/18032c1))
* fix package.json files ([28b1751](https://bitbucket.org/openagenda/oa/commits/28b1751))
* fix storybooks ([4737113](https://bitbucket.org/openagenda/oa/commits/4737113))
* force to use the good react-redux context ([fee9914](https://bitbucket.org/openagenda/oa/commits/fee9914))
* functional webapp ([793ca92](https://bitbucket.org/openagenda/oa/commits/793ca92))
* inbox-apps: add a spinner when an action is triggered ([2d35704](https://bitbucket.org/openagenda/oa/commits/2d35704))
* inbox-apps: already display resolvedWith if exist ([79d3b1c](https://bitbucket.org/openagenda/oa/commits/79d3b1c))
* inbox-apps: don't use legacy lifecycle methods ([abb4c21](https://bitbucket.org/openagenda/oa/commits/abb4c21))
* inbox-apps: fix close confirmation modal's title ([c389722](https://bitbucket.org/openagenda/oa/commits/c389722))
* inbox-apps: fix createConversation ([ca804da](https://bitbucket.org/openagenda/oa/commits/ca804da))
* inbox-apps: fix language ([0a9a855](https://bitbucket.org/openagenda/oa/commits/0a9a855))
* inbox-apps: fix lazyInbox ([8f31c82](https://bitbucket.org/openagenda/oa/commits/8f31c82))
* inbox-apps: fix lazyInbox ([3d283d0](https://bitbucket.org/openagenda/oa/commits/3d283d0))
* inbox-apps: fix sendMessage ([7aae421](https://bitbucket.org/openagenda/oa/commits/7aae421))
* inbox-apps: fix undefined submitting ([d700db3](https://bitbucket.org/openagenda/oa/commits/d700db3))
* inbox-apps: fix usage of query ([b1ccffe](https://bitbucket.org/openagenda/oa/commits/b1ccffe))
* inbox-apps: fixes ([9d3a3d7](https://bitbucket.org/openagenda/oa/commits/9d3a3d7))
* inbox-apps: fixs ([d32bded](https://bitbucket.org/openagenda/oa/commits/d32bded))
* inbox-apps: remove test-app ([0bd69e5](https://bitbucket.org/openagenda/oa/commits/0bd69e5))
* inbox-apps: tweaks ([f5ee7a4](https://bitbucket.org/openagenda/oa/commits/f5ee7a4))
* inbox-apps: use directly the location instead of history.location ([832c6bc](https://bitbucket.org/openagenda/oa/commits/832c6bc))
* lang from extraProps ([ebe6213](https://bitbucket.org/openagenda/oa/commits/ebe6213))
* Merge branch 'master' into webapp ([dc1e964](https://bitbucket.org/openagenda/oa/commits/dc1e964))
* Merge branch 'master' into webapp ([e94b577](https://bitbucket.org/openagenda/oa/commits/e94b577))
* Merge branch 'master' of bitbucket.org:openagenda/oa into webapp ([033351b](https://bitbucket.org/openagenda/oa/commits/033351b))
* Merge branch 'master' of bitbucket.org:openagenda/oa into webapp ([fb2d4d3](https://bitbucket.org/openagenda/oa/commits/fb2d4d3))
* Merge branch 'master' of bitbucket.org:openagenda/oa into webapp ([2d7383c](https://bitbucket.org/openagenda/oa/commits/2d7383c))
* Merge branch 'master' of bitbucket.org:openagenda/oa into webapp ([44f1fcc](https://bitbucket.org/openagenda/oa/commits/44f1fcc))
* Merge branch 'master' of bitbucket.org:openagenda/oa into webapp ([356677c](https://bitbucket.org/openagenda/oa/commits/356677c))
* prefer react-utils for inbox app ([e8263af](https://bitbucket.org/openagenda/oa/commits/e8263af))
* react-utils and dependents refacto ([2889e4f](https://bitbucket.org/openagenda/oa/commits/2889e4f))
* react-utils: move apiClient interceptors behind an option legacy ([f087ea9](https://bitbucket.org/openagenda/oa/commits/f087ea9))
* remove deprecated @babel/polyfill and upgrade to core-js@3 ([e006882](https://bitbucket.org/openagenda/oa/commits/e006882))
* remove useless babel-core@bridge ([f2d2d91](https://bitbucket.org/openagenda/oa/commits/f2d2d91))
* remove useless imports ([8d36861](https://bitbucket.org/openagenda/oa/commits/8d36861))
* rename babel-preset-openagenda to @openagenda/babel-preset ([1da3284](https://bitbucket.org/openagenda/oa/commits/1da3284))
* render in lazyInbox/load ([7e665e5](https://bitbucket.org/openagenda/oa/commits/7e665e5))
* replace react-router with react-router-dom ([e87088e](https://bitbucket.org/openagenda/oa/commits/e87088e))
* tweaks ([a1b606a](https://bitbucket.org/openagenda/oa/commits/a1b606a))
* update Jest multi projects config ([41ced2a](https://bitbucket.org/openagenda/oa/commits/41ced2a))
* upgrade babel ([444b969](https://bitbucket.org/openagenda/oa/commits/444b969))
* upgrade babel dependencies ([97b3057](https://bitbucket.org/openagenda/oa/commits/97b3057))
* upgrade dependencies ([193c6a0](https://bitbucket.org/openagenda/oa/commits/193c6a0))
* upgrade dependencies ([5c270a6](https://bitbucket.org/openagenda/oa/commits/5c270a6))
* upgrade deps ([925ed0f](https://bitbucket.org/openagenda/oa/commits/925ed0f))
* upgrade jest and core-js ([21c2768](https://bitbucket.org/openagenda/oa/commits/21c2768))
* upgrade react ([0c64635](https://bitbucket.org/openagenda/oa/commits/0c64635))
* upgrade react ([9096056](https://bitbucket.org/openagenda/oa/commits/9096056))
* upgrade react-router deps ([94402fd](https://bitbucket.org/openagenda/oa/commits/94402fd))
* upgrade react-router to v5 ([25bd2aa](https://bitbucket.org/openagenda/oa/commits/25bd2aa))
* upgrade react-waypoint ([de08cbb](https://bitbucket.org/openagenda/oa/commits/de08cbb))
* upgrade some deps ([33a049a](https://bitbucket.org/openagenda/oa/commits/33a049a))
* upgrade some deps ([dd48588](https://bitbucket.org/openagenda/oa/commits/dd48588))
* upgrade some deps ([1d10a03](https://bitbucket.org/openagenda/oa/commits/1d10a03))
* upgrade some deps ([998baed](https://bitbucket.org/openagenda/oa/commits/998baed))
* upgrade some deps and remove unused deps ([0c212fb](https://bitbucket.org/openagenda/oa/commits/0c212fb))
* use the same lodash ([aeae186](https://bitbucket.org/openagenda/oa/commits/aeae186))
* inboxes: fix resolved conversations ([a1a04ed](https://bitbucket.org/openagenda/oa/commits/a1a04ed))
* WIP: add SSR ([05dcb41](https://bitbucket.org/openagenda/oa/commits/05dcb41))
* WIP: update bug ([57348a5](https://bitbucket.org/openagenda/oa/commits/57348a5))
