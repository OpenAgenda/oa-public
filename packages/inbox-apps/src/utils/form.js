...
export function renderField(props) {
  ...
  if (props.displayError) {
    displayError = props.displayError(meta);
  } else if (errorOnDirty) {
    displayError = dirty && touched;
  } else {
  ...
  const className = cn('form-group', classNameGroup, {
    'has-error has-feedback': displayError && error,
  });

  const errorClassName = cn('text-danger', {
    'pull-left': max,
  });

  const maxClassName = cn('text-right', {
  ...
  return (
    <div className={className}>
      ...
      {subLabel}
      {content}
      {displayError && displayFeedback && error && (
        <span className="form-control-feedback">
          <i className="fa fa-times" aria-hidden="true" />
        </span>
      )}
      {displayError && error && (
        <div className={errorClassName}>{getErrorLabel(error)}</div>
      )}
      {max && <div className={maxClassName}>{max - value.length}</div>}
    </div>
  );
}
