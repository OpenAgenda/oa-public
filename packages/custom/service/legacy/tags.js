...
async function load(agendaEventId) {
  ...
  return (
    await knex(`${schemas.agendaTag} as at`)
      .select('tag')
      .leftJoin(
        `${schemas.agendaEventTag} as aet`,
        'at.id',
        'aet.review_tag_id',
      )
      .where('aet.review_article_id', agendaEventId)
  ).map((r) => r.tag);
}
...
function parse(fields, legacyTags) {
  ...
  fields.forEach((field) => {
    ...
    const matchingOptions = options.filter((o) =>
      legacyTags.includes(
        _.isObject(o.label)
          ? _.get(o.label, 'fr', _.first(_.keys(o.label)))
          : o.label,
      ));
...
async function set(agendaEventId, fields, data) {
  ...
  const labels = fields.reduce(
    (accu, field) =>
      accu.concat(
        _.flatten(
          field.options.map((o) =>
            (_.isString(o.label) // sometimes isn't multilingual
              ? o.label
  ...
  const pickedLabels = fields.reduce((picked, field) => {
    ...
    const matchingLabels = field.options
      ...
      .map((o) => o.label)
      .map((label) =>
        (_.isString(label) // sometimes isn't multilingual
          ? [label]
  ...
  const pickedLegacyTags = legacyTags.filter((lt) =>
    pickedLabels.includes(lt.tag));

  const toBeRemovedLegacyTags = legacyTags.filter(
    (lt) => !pickedLabels.includes(lt.tag),
  );

  await knex(schemas.agendaEventTag)
    .delete()
    .where('review_article_id', agendaEventId)
    .whereIn(
      'review_tag_id',
      toBeRemovedLegacyTags.map((t) => t.id),
    );

  let inserts = 0;

  for (const { id } of pickedLegacyTags) {
    const current = await knex(schemas.agendaEventTag).first('id').where({
