...
describe('opencage', () => {
  ...
  describe('forward', () => {
    ...
    describe('Métropole de Lyon, département du Rhône', () => {
      it('Maillane is in "Bouches-du-Rhône" department', async () => {
        const result = await geocode('11 Avenue Lamartine, Maillane', {
          countryCode: 'FR',
          first: true,
        });
      ...
      it('Bron is in "Métropole de Lyon" department', async () => {
        const result = await geocode('20 Rue Villard, 69500 Bron', {
          countryCode: 'FR',
          first: true,
          language: 'fr',
        });
      ...
      it('Taluyers is in "Rhône" department', async () => {
        const result = await geocode("47 montée de l'église 69440 Taluyers", {
          countryCode: 'FR',
          first: true,
          language: 'fr',
        });
      ...
      it('43 rue des Hérideaux, Lyon is in "Métropole de Lyon" department', async () => {
        const result = await geocode('43 rue des Hérideaux, Lyon', {
          countryCode: 'FR',
          first: true,
          language: 'fr',
        });
    ...
    describe('DOM-TOM', () => {
      it('Saint Pierre et Miquelon addresses respond to PM country code', async () => {
        const res = await geocode(
          'Rue du 11 novembre B.P. 4208, Saint-Pierre, Saint-Pierre-et-Miquelon',
          {
            countryCode: 'PM',
            language: 'fr',
          },
      ...
      it('Nouvelle Calédonie addresses respond to RE country code', async () => {
        expect(
          (
            await geocode(
              'Maison Célières, 21, route du Port-Despointes, Faubourg-Blanchot, NOUMEA, Nouvelle-Calédonie',
              {
                countryCode: 'NC',
                language: 'fr',
              },
    ...
    describe('Hong Kong', () => {
      it('Finds an address in Hong Kong', async () => {
        const result = await geocode(
          '11 Man Kwong Street, Central, Hong Kong',
          {
            countryCode: 'HK',
            language: 'fr',
            first: true,
  ...
  describe('reverse', () => {
    ...
    it('In Guyane', async () => {
      expect(
        _.pick(
          await geocode.reverse(5.6688522, -53.7819599, {
            first: true,
            language: 'fr',
          }),
          [
