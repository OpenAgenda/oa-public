...
describe('12 - updateSource', () => {
  ...
  test(
    'if agenda is source, calls fn to update source, providing aggregator agenda and source entry',
    () => {
  ...
  test(
    'fix: enqueueLoadSourceEvaluates is given the aggregator and source rules',
    () => {
      expect(tracker.calls[3].args[0].sourceRules).toEqual([{ query: { categorie: 2 } }]);
      expect(tracker.calls[3].args[0].aggregatorRules).toEqual([{ actions: [{ state: 1 }] }]);
    }
  );
