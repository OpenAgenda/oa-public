...
describe('05_02 - utils - rules', () => {
  ...
  describe('basic usage', () => {
    ...
    test(
      'a ruleset that matches a given value returns the data to be associated to the event when added to an aggregator',
      () => {
        const input = {
          thematiques: 1
        };
    ...
    test(
      'if provided schemas share the same fields, the values are maintained in response',
      () => {
        const input = {
          thematiques: 1
        }
    ...
    test(
      'a matching rule with actions applies actions to provided values to generate result',
      () => {
        const input = {
          thematiques: 12
        };
    ...
    test(
      'if state is set multiple actions, last state is applied state only if $set is used',
      () => {
        const ruleset = [{
          actions: [{
            state: 1
  ...
  describe('query', () => {
    ...
    test(
      'a query matches if at least one of the defined values of the query is in the provided values',
      () => {
        const input = {
  ...
  describe('actions', () => {
    ...
    test(
      'an action overwrites pre-existing values when they are already set',
      () => {
        const input = {
          thematiques: 12
        };
    ...
    test(
      'same as previous, with legacy action structure (overwrite preexisting)',
      () => {
        const input = {
          thematiques: 12
        };
    ...
    test(
      '$push operations are ignored when set in first action of given field',
      () => {
        expect(rules([{
          actions: [{
            thematiques: {
  ...
  describe('label filters', () => {
    ...
    describe('without actions', () => {
      ...
      test(
        'tag evaluate does not pass if data does not have tag specified in query',
        () => {
          expect(evaluate({
            title: 'A thing',
            tags: [3]
      ...
      test(
        'tag evaluate passes event if query does not match if required is false',
        () => {
          expect(rules({
            query: {
              tags: ['Tag1']
    ...
    describe('with actions', () => {
      ...
      test(
        'if data matches rule and a transform is specified, it is applied',
        () => {
          expect(evaluate({
  ...
  describe('location filters', () => {
    ...
    test(
      'if one location evaluated field does not match, the rule does not match',
      () => {
        expect(evaluate({
          location: {
            name: 'La boutique',
    ...
    test(
      'when multiple locations are specified in the same rule, operand is OR',
      () => {
        expect(rules([{
          query: {
            location: [{
    ...
    test(
      'multiple values can be specified in the same filter field for an OR evaluation',
      () => {
        expect(rules([{
          query: {
            location: {
  ...
  describe('legacy', () => {
    ...
    test(
      'evaluation based on boolean value does not pass if boolean is different',
      () => {
        expect(rules([{
          query: {
            intercommunal_interest: true
