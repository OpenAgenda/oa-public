...
describe('08 - removeEvent', () => {
  ...
  test(
    'if reference shows add by aggregation and source is last source refered, unreferenceEvent is called',
    async () => {
  ...
  test(
    'if reference shows that other sources reference event, then updateSourcePaths is called',
    async () => {
      const tracker = Tracker();
      await removeEvent({
        getEventReference: tracker('getEventReference', {
  ...
  test(
    'if reference shows that event was not added to aggregator agenda through aggregation, then updateSourcePaths event if source is last',
    async () => {
      const tracker = Tracker();
      await removeEvent({
        getEventReference: tracker('getEventReference', {
  ...
  test(
    'if unreference fails, result provides success bool at false and errors',
    async () => {
      const tracker = Tracker();
