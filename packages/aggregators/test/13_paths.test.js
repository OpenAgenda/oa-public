...
describe('13 - paths', () => {
  ...
  describe('updateIsRequired', () => {
    ...
    test(
      'if no sourcePaths are registered in reference, it needs to be updated',
      () => {
    ...
    test(
      'sourcePaths are registered in reference, but provided sourcePath includes a path unknown by reference',
      () => {
        expect(updateIsRequired([[111, 222, 333]], [[111, 222], [100, 6, 222]], 333)).toBe(true);
      }
    );

    test('all paths of source are known to reference', () => {
      expect(
        updateIsRequired([[111, 222, 333], [100, 6, 222, 333]], [[111, 222], [100, 6, 222]], 333)
      ).toBe(false);
    });
  ...
  describe('getAmended', () => {
    ...
    test(
      'source paths are appended to pre-existing paths unrelated to source',
      () => {
  ...
  describe('getFiltered', () => {
    ...
    test(
      'specified source can be deeper down in path for the path to be removed',
      () => {
