...
export function createSaturationScale(
  tweak: number = 0,
  fromStop: number = DEFAULT_STOP,
) {
  const stops = DEFAULT_STOPS;
  const index = stops.indexOf(fromStop);

  if (index === -1) {
    throw new Error(`Invalid key value: ${fromStop}`);
  }
...
export function createHueScale(
  ...
  if (index === -1) {
    throw new Error(`Invalid parameter value: ${fromStop}`);
  }
