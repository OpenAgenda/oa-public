...
import { Icon, IconProps, useToken } from '@openagenda/uikit';
...
const OAIcon = forwardRef<SVGSVGElement, OAIconProps>(function OAIcon(
  {
    withShadow = false,
    primaryColor,
    secondaryColor,
  ...
  const [
    primaryC = 'currentColor',
    secondaryC = 'currentColor',
    tertiaryC = 'currentColor',
  ] = useToken('colors', [primaryColor, secondaryColor, tertiaryColor]);

  const viewBoxHeight = withShadow ? 216.004 : 185.594;

  return (
    <Icon ref={ref} {...props}>
      <svg viewBox={`0 0 146.278 ${viewBoxHeight}`}>
        <path
