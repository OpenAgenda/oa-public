...
- Modéliser le **socle commun** Event/Location/Agenda + champs additionnels nestés sous `additionalFields` (`Record<string, CustomFieldValue>`) ; documenter l'endpoint de schéma par agenda. Cf. §9 décision 5.
- Séparer explicitement les routes publiques (tagging `x-internal` pour le reste).
- Unifier l'**auth** publique sur `Authorization: Bearer` (clé préfixée ou token), scopes mappés aux tiers, via le plugin **`@better-auth/api-key`** (table `apikey` native — BA `1.6.11`) ; OAuth2 `client_credentials` via `oidc-provider`/`oauth-server` planifié (hors slice). Détail, séquence de déploiement (D0→D6) et chantiers induits : `docs/plan-slice-auth-v3.md` et §9 décision 4.
...
4. **Auth cible** : ✅ **tranchée + en cours d'implémentation** — la direction ci-dessous a été reprise et **affinée** dans `docs/plan-slice-auth-v3.md` (le plan de référence de la tranche auth) ; D0→D5a sont mergés. ⚠️ **La sous-décision « better-auth » ci-dessous a été inversée à l'exécution** (cf. bullet « Better-auth » corrigé) : on a finalement **adopté le plugin `@better-auth/api-key`** au lieu d'étendre le store maison. Direction :
   ...
   - **Better-auth** : ✅ **direction retenue (revue depuis l'analyse initiale)** — le plugin `@better-auth/api-key` **existe** dans la ligne `1.6.11` (package séparé, même découpage que `@better-auth/core`). On l'a donc **adopté** (bump BA `1.6.9 → 1.6.11`), table `apikey` native (hashée, multi-clé, `referenceId`, `permissions`, rate-limit), et **supprimé** le store maison (`api_key_set` + `packages/keys`). _(L'analyse initiale recommandait l'inverse — étendre le store maison — sur la foi que BA 1.6.9 n'avait pas ce plugin ; vérification faite, le plugin est disponible et a été préféré. Détails et faits vérifiés : `plan-slice-auth-v3.md` §2-3.)_ Côté OAuth `client_credentials`, inchangé : `oidc-provider`/`oauth-server` planifié (hors slice, différé à la Phase 7 SSO).
   - **Pré-requis / chantiers induits** : multi-clé + révocation par clé = **nouveau modèle de données** ✅ (table `apikey`) ; invalidation de cache (révocation < 1 h) ; unifier la vérif blacklist sur les deux chemins ✅ (D0) ; rate-limit par clé (porté par le plugin, activé en D6) ; **`?key=` en query conservé** côté legacy v2 (fenêtre de dépréciation liée à l'EOL v2) ; le **sel HMAC `'okilydokily'`** n'est **pas** roté mais **gelé** avec le flux `tk-` de v2 (retrait lié à l'EOL v2, cf. `plan-slice-auth-v3.md` D4) — v3 n'introduit jamais `tk-`.
5. **Champs additionnels** : ✅ **reco** (vérifiée dans le code) — **nester les champs additionnels sous une clé dédiée `additionalFields`** en v3, socle natif typé et fermé au top-level. _(MAJ : la clé, d'abord nommée `custom`, a été renommée `additionalFields` pour aligner le contrat v3 sur le vocabulaire maison — `core`, `event-search`, front `next` disent tous `additionalFields` ; `custom` n'existait qu'en v3. La décision de **nester** est inchangée ; seul le mot change. Le filtre est `additionalFields[<field>]`. Côté `core`, la clé de requête interne reste `query.custom`.)_
