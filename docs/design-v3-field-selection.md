...
### Expérimentation (20 juin 2026) — branche `experiment/v3-projection-synthesis` (depuis `feat/v3-fields-spec-driven`)

But : **voir le bout du chemin** des lots reportés A (#1+#2) et C (#3) et **mesurer le coût réel**, sur une branche jetable. Protocole re-rejoué avant de coder : 4 agents de recherche (sémantique des 3 services, inventaire consommateurs) + 3 agents de review adverse (« codex » : réfuter la valeur/sécurité).

**Re-vérification de Lot A — plus tranchée que le 19 juin, et corrige une erreur :**

1. **`canEditEvent` est fail-CLOSED, pas fail-open.** Lecture directe (`canEditEvent.js:57,85`) : `if (isDraft && user.uid === ownerUid)` — un `ownerUid` absent rend `user.uid === undefined` faux → accès **refusé**. Le 19 juin écrivait « échec en mode ouvert / faille » : **faux**. Le risque réel n'est pas une faille de sécurité mais une **régression fonctionnelle** (utilisateurs refusés à tort). Toujours un breaking change, mais re-qualifié.
2. **Le renommage des signatures de service est non seulement de la churn, il est _nuisible_ — fait nouveau décisif.** `includeFields` d'event-search porte un **alias public `if`**, lu par l'API **v2 `/events`** (`api/index.js:1281` : `req.query.includeFields ?? req.query.if`). Renommer l'option service `includeFields→only` casserait **silencieusement la surface publique v2**. Ce n'est donc pas du vocabulaire interne. À cela s'ajoutent ~26 consommateurs (pas ~14) sur 6+ packages et des pièges de régression : les auto-ajouts `timings`/`valid`/`removed` sont conditionnés sur la présence de `requested` (`defineIncludes.js:31-56`, `search.js:192-199`).
3. **`includeFields` veut dire des choses _différentes_ selon le service** : override chez event-search, **additif** chez agenda-search (`onlyIncludeFields` y est l'override), restrictif chez agenda-locations (`include` y est le force-keep keyset, déjà la sémantique additive cible). C'est ça la vraie incohérence — interne, invisible au contrat public.
4. **Un `include` additif sur event-search = YAGNI pur.** 0 des 11 consommateurs d'event-search veulent ajouter au-dessus du palier ; tous veulent une projection étroite (override). L'ajouter par symétrie introduit un piège (un `include` que `defineIncludes` ne fusionne pas serait silencieusement ignoré, et casserait les auto-ajouts).

**Décision.** La clause de sortie du goal s'applique à la partie _nuisible_ : **on n'a PAS renommé les signatures de service** (alias v2 `if`, ~26 consommateurs, pièges auto-ajouts) ni ajouté l'additif YAGNI. On a implémenté à la place **l'incarnation _sûre_ de la synthèse, à la frontière v3** — là où la convergence a réellement sa place (cf. point 1 du 19 juin : le contrat est déjà unifié en sortie).

**FAIT — synthèse de frontière (`applyProjection`).** Le v3 exprime désormais **une seule intention de projection** — « projette seulement ces champs » — via `applyProjection(options, fields, descriptor, extra)` (`selectFields.js`). Chaque descripteur SELECT (co-localisé avec son mapper) porte le **nom de l'option native** du service : `EVENT_SELECT.option`/`LOCATION_SELECT.option = 'includeFields'`, `AGENDA_SELECT.option = 'onlyIncludeFields'`. Les **4 sites d'appel** v3 (agendas, events, locations, /me) passent de trois formes inline divergentes à un seul appel ; `selectionToIncludes` (le traducteur de paths) reste dessous, inchangé. **Aucun service, consommateur ou test hors api-v3 touché ; alias v2 `if` intact ; sortie publique #183/#184 identique** (intégration verte : events 83, agendas 34, locations + /me 33 ; unité 54). Aucun package `public/` modifié → pas de changeset.

**Constat honnête de la frontière.** Le v3 n'utilise jamais que l'axe **restrictif** (`only`) ; l'axe **additif** (`include`) reste un sujet de consommateur _interne_ (agenda-search `includeFields`, locations `include` force-keep) que les services gardent pour eux. Le « couple `{include, only}` » se réduit, au bord, à `only` seul. La convergence des _signatures de service_ reste reportée au chemin **II** (un 2ᵉ consommateur service réel) — désormais avec la garde supplémentaire : ne pas toucher `includeFields` d'event-search sans gérer l'alias public v2 `if`.

**Lot C (#3) — IMPLÉMENTÉ et MESURÉ empiriquement** (la branche est jetable : on construit pour mesurer le coût réel plutôt que l'estimer). 2 agents de recherche + 2 agents adverses d'abord, puis implémentation.

_Prédictions de la review adverse :_ (1) **ratio défavorable** — supprimer `pickSelected`/`pathsToTree`/`pickTree`/`selectionTree`/`treeCache` (~53-68 lignes centralisées) pour les disperser en émissions conditionnelles par mapper ; (2) **dépendance cassée silencieuse** — `meAgendas.js` `AGENDA_UNIVERSE = fieldNamesOf(mapAgendaDetailed)` et le test de drift sondent `mapItem({})` en comptant sur l'émission de l'univers complet ; un mapper « sparse par défaut » la casserait ; (3) **filet mince** — l'intégration ne couvre pas le trim de tableau partiel / sous-objet partiel ; (4) **zéro perf** — `pickTree` est marginal devant `mapItem` ; (5) **piège du chemin sans-sélection (95% du trafic)**.

_Implémentation réelle._ `compileSelection` (arbre construit **une fois par page** dans l'envelope, la `WeakMap treeCache` disparaît) + `trimValue` (= l'ancien `pickTree`, **inchangé**) exportés ; chaque mapper gagne un param `tree` ; **contrat clé : `tree === null` ⇒ forme complète empty-as-empty** (donc la sonde `mapItem({})` reste intacte). `mapEvent` saute la _construction_ des champs top-level non sélectionnés (vrai « pousser = trimer ») + gère le bag ; `mapAgenda`/`mapLocation`/`meAgendas` (plats) font build-then-`trimValue`.

_Mesure._ **+180 / −88 sur 9 fichiers** ; **96 tests unitaires + 150 d'intégration verts** ; sortie **strictement identique** à la passe `pickSelected` (events 83, agendas 34, locations 23, /me 10).

_Verdict empirique — affine la prédiction :_

- **Prédiction (2) ÉVITÉE, pas fatale.** En gardant `tree === null ⇒ full`, la sonde d'univers, `AGENDA_UNIVERSE`, le test de drift et le chemin sans-sélection (95%) restent **intacts** (tout vert). Le « break silencieux » n'arrive que si on régresse vers sparse-par-défaut — ce que le contrat null=full empêche. Donc la réécriture est **sûre**, pas la catastrophe annoncée.
- **Prédiction (1) CONFIRMÉE.** « Une couche au lieu de deux » est **faux** : `trimValue` EST la 2ᵉ couche, juste **relocalisée**. Retirer l'orchestration centralisée de `selectFields.js` a **ajouté de la plomberie à 7 fichiers** (3 envelopes + 3 mappers + meAgendas) ; bilan **net positif en lignes** (+92 hors-tests inclus). L'algorithme de trim est identique.
- **Prédiction (4) CONFIRMÉE.** Le seul changement comportemental réel — `mapEvent` ne _construit_ plus les champs non demandés — a un gain **marginal** : le pushdown `_source` ES évite déjà de _fetcher_ ces champs, donc le mapper les reçoit rarement.

**Décision Lot C : implémenté pour la mesure, puis _reverté sur la branche_ (code retiré, ces findings conservés).** L'expérience prouve que la réécriture est faisable et sûre, mais qu'elle **disperse sans réduire** (couche identique relocalisée + un param `tree` sur chaque signature de mapper) pour un gain perf nul. `pickSelected` reste. La valeur de ce lot est la **preuve mesurée**, pas le code — d'où le revert immédiat, qui laisse la leçon écrite ici sans alourdir les mappers.

### Bilan d'expérimentation (20 juin 2026)

**Ce qui a changé (livré).** Une seule chose de code : les **4 sites de pushdown v3** (agendas/events/locations/me), jusque-là trois formes inline divergentes (`onlyIncludeFields = …` / `const includeFields = … ; if (includeFields) …` / `includeFields = …`), passent par **un seul `applyProjection(options, fields, descriptor, extra)`** ; chaque descripteur SELECT gagne `option` (son nom d'option native). `selectionToIncludes` reste le traducteur de paths, inchangé.

**Coût vs gain.** Gain : un vocabulaire de projection unique au bord, le « quelle option native » possédé par le descripteur (co-localisé avec le mapper) au lieu d'être codé en dur à chaque route ; la garde `null`/bag-vide centralisée (un site au lieu de trois). Coût : +1 helper (~15 lignes) + un champ `option` ×3 descripteurs. **Consommateurs réellement touchés hors api-v3 : zéro.** Sortie publique #183/#184 : identique (intégration verte).

**Les deux gros mouvements.** Convergence des _signatures_ de service (A#1) : **nuisible**, non fait — l'alias public **v2 `if`** d'event-search, ~26 consommateurs internes, pièges d'auto-ajouts. Mappers projection-aware (C) : **implémenté et mesuré** sur la branche (96+150 tests verts, sortie identique, +180/−88 sur 9 fichiers) → **sûr mais sans valeur nette** : la couche de trim est relocalisée, pas supprimée (`trimValue` = l'ancien `pickTree`), et le seul gain (mapEvent ne construit plus le non-demandé) est marginal car le pushdown ES évite déjà de fetcher. Recommandation : **revert avant `main`**. La valeur de C est la preuve empirique, pas le merge.

**Coût empirique de C.** Commit séparé `?fields=` projection-aware : retire `pickSelected`/`selectionTree`/`treeCache` de `selectFields.js` au profit de `compileSelection` (1×/page dans l'envelope) + `trimValue` ; ajoute un param `tree` à `mapEvent`/`mapAgenda`/`mapLocation`/`meAgendas` et aux 3 envelopes. Contrat préservé : `tree === null ⇒ forme complète` (sonde d'univers, drift, chemin sans-sélection intacts).

**Verdict final vs review du 19 juin.** L'expérimentation **confirme et affine** : (a) corrige une erreur — `canEditEvent` est **fail-closed**, pas fail-open (risque fonctionnel, pas une faille) ; (b) ajoute le **bloqueur dur** que le 19 juin n'avait pas vu — l'alias public v2 `if` rend le renommage d'event-search non-cosmétique (A#1 reste non fait) ; (c) **mesure C en le construisant** plutôt qu'en l'estimant : sûr (96+150 tests verts, sortie identique, sonde d'univers préservée par le contrat null=full — la « dépendance cassée » prédite est _évitable_) mais sans valeur nette (couche de trim relocalisée, pas supprimée ; gain perf nul) → **à reverter avant `main`**. Le **seul** incrément sûr ET de valeur du « bottom-up » est la synthèse de _frontière_ (Lot A, mergeable). C (preuve faite) et le chemin **II** (A#1) attendent un déclencheur réel, pas un principe.

### Variante « converger sur `only` » (aliasée, non-breaking) — explorée, **implémentée et mesurée**, écartée (20 juin 2026)

Question posée après l'expérimentation : et si on faisait A#1 **sans casser**, en _ajoutant_ `only` aux 3 validateurs comme alias normalisé de l'option restrictive existante (event : `only ?? includeFields ?? if` ; agenda-search : `only ?? onlyIncludeFields` ; locations : `only ?? includeFields`), en gardant les anciens noms en alias dépréciés, et en basculant les 3 descripteurs v3 sur `option: 'only'` — **sans migrer les ~26 consommateurs** (ils gardent `includeFields`, qui marche) et **sans** introduire d'`include` additif public ?

Protocole rejoué : 3 agents de recherche (sémantique + faisabilité d'alias des 3 validateurs) + 1 agent adverse. Trois faits ré-établis dans le code : (a) event-search `includeFields` est **override**, normalisé en **un seul point** (`validateSearchOptions.js:98`) → l'alias serait propre et localisé ; (b) **agenda-search n'a que 2 consommateurs** (un test + la frontière v3) — le « touche pas mal de consommateurs » ne vise en fait qu'event-search, et ses consommateurs restent intacts via l'alias ; (c) l'axe additif `include` **n'est pas uniforme** (interne keyset en locations, inexistant en event-search) — rien à converger là.

**Verdict : DON'T (abandonner + documenter).** Même issue que Lot C, pour des raisons structurelles décisives :

1. **La convergence existe déjà, une couche plus haut.** `applyProjection` indirecte le nom d'option via le descripteur du mapper ; les appelants v3 ne tapent jamais `includeFields`/`onlyIncludeFields`. Le but « vocabulaire interne uniforme » est **déjà atteint au seul endroit qui en bénéficie** (la frontière v3, cf. synthèse de frontière ci-dessus). Renommer en `only` = **zéro delta runtime** (échange de littéraux + alias qui re-pointe le même chemin de code).
2. **Ça _dégrade_ la surface au lieu de l'unifier.** event-search garde `if` **ET** `includeFields` **pour toujours** (contrat public v2, irretirable). Après le changement, un lecteur doit comprendre **`if` ET `includeFields` ET `only`** comme synonymes — là où il en a deux aujourd'hui. **Un refactor d'unification qui ne peut jamais retirer l'ancien vocabulaire n'est pas une unification, c'est de l'accrétion.** On paie la taxe synonyme sans jamais encaisser le nettoyage.
3. **Risque net pour rien** : 2 nouvelles chaînes de précédence `??`/`||` (event-search a déjà un skew `||` vs `??` entre `validateSearchOptions.js:98` et `api/index.js:1281`), et un 3ᵉ nom « plausible » qui élargit l'ensemble des typos résolvant silencieusement.

La convergence des _signatures de service_ reste donc reportée au **chemin II** sous sa forme « vrai besoin » (un 2ᵉ consommateur service réel qui veut un `select` public), **pas** sous la forme alias-cosmétique. Le seul follow-up à valeur réelle du bottom-up reste le typage additif `Pick<T, Fields>` noté dans `selectFields.js:45-51`. `only` aliasé : non.

**Implémenté puis mesuré (même protocole que Lot C).** La branche étant jetable, on a tout de même _construit_ la variante pour mesurer son coût réel plutôt que l'estimer. `only` ajouté en alias normalisé aux **3 validateurs** (`validateSearchOptions.js` event : `only ?? (includeFields || if)` ; `validators/options.js` agenda-search : wrapper `only ?? onlyIncludeFields` ; `validateListOptions.js` + `cleanGetOptions.js` locations : `only ?? includeFields`), et les **3 descripteurs SELECT** v3 basculés sur `option: 'only'`. **Diff total : +46/−18 sur 7 fichiers** (+ ~8 lignes d'assertions de test mises à jour). **Mesure : tout vert** — event-search 488, agenda-search 58, agenda-locations 254 (les 8 échecs `03_create` sont **pré-existants**, identiques baseline), v3 unitaire 214, **v3 intégration 146** (events 79, agendas 34, locations 23, me 10). **Sortie publique strictement identique** : le `?fields=` traverse `.only` → chaque validateur le normalise vers son nom legacy → projection inchangée.

_Verdict empirique = confirme la prédiction, intégralement._ La variante est **faisable, sûre et sans valeur** : zéro delta runtime (sortie identique), zéro consommateur migré, zéro surface publique changée — un **renommage littéral + plomberie d'alias**. L'accrétion prédite est concrète : event-search expose désormais `if` + `includeFields` + `only`, **trois synonymes** pour un axe sur une surface publique qu'on ne nettoiera jamais. La convergence « visible » se réduit à trois littéraux de descripteur passant de noms natifs à `'only'` — exactement ce que l'indirection par descripteur (Lot A) rend déjà inutile. **Décision : implémenté pour la mesure, puis _reverté sur la branche_ (code retiré, ce constat conservé).** Comme Lot C, la valeur est la preuve mesurée, pas le merge.
