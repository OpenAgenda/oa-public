...
**Re-vérification de Lot C (#3) — REPORTÉ, le gain ne paie pas le risque.** 2 agents de recherche (structure des mappers + couverture de test) + 2 agents adverses (réfuter « une couche au lieu de deux » + pièges de correction). Convergence nette : **Lot C _déplace et multiplie_ la complexité au lieu de la réduire.**

1. **Ratio défavorable.** Supprimer `pickSelected`/`pathsToTree`/`pickTree`/`selectionTree`/`treeCache` = **~53-68 lignes centralisées, génériques, testées isolément** (`selectFields.js`). Les remplacer par des mappers projection-aware = **~86 décisions conditionnelles distribuées** sur 4 fichiers (mapEvent `mapWithFields` + CLEANERS, mapAgenda ×3 formes, mapLocation ×2, meAgendas graft) — ~1,6× la complexité, sans réutilisabilité. Une passe générique → N émissions conditionnelles éparses.
2. **Dépendance cassée, silencieuse en prod (le plus grave).** `meAgendas.js:29` `AGENDA_UNIVERSE = new Set(fieldNamesOf(mapAgendaDetailed))` et le test de drift spec↔mapper (`90_unit_apiV3_selectFields.test.js:104-122`) **sondent `mapItem({})` en comptant sur l'émission de l'univers complet**. Un mapper projection-aware (qui n'émet que le sélectionné) casse cette sonde → `AGENDA_UNIVERSE` devient incomplet → des champs sont **silencieusement retirés du pushdown /me**, sans erreur ni test rouge.
3. **Filet de sécurité mince.** L'intégration `?fields=` ne couvre PAS : le trim d'un **tableau partiel** (`timings.begin` → `[{begin}]`), le **sous-objet partiel** (`location.uid,location.name`), ni l'**absence de clés en trop**. Ces cas ne sont tenus aujourd'hui que par les tests unitaires `pickTree` — qui **disparaîtraient** avec la suppression. La réécriture retire la couche au moment précis où l'on perd ses tests.
4. **Aucun gain de perf.** `mapItem` (coercition de ~50 champs + cleaners) **domine** ; `pickTree` (~5-10 clés, arbre caché par page via `treeCache` WeakMap, 1 build/100 items) est marginal. Supprimer la 2ᵉ passe économise un objet intermédiaire microscopique.
5. **Piège majeur : le chemin _sans_ sélection (≈95% du trafic).** Un mapper projection-aware doit **toujours** émettre la forme complète empty-as-empty quand `fields === null`. S'il régresse vers « sparse par défaut », il casse la majorité du trafic (pagination keyée par `uid`, schéma figé `required`). Risque **asymétrique** : réécrire pour le cas sparse (5%) en exposant le cas plein (95%). S'ajoutent les pièges `uid` toujours-retenu, timings calculés, priorité bare>deep, double responsabilité CLEANERS×trim, graft role/private de /me.

**Décision Lot C : reporté** (clause de sortie). `pickSelected` reste — c'est une passe générique, centralisée, bien testée, au coût négligeable ; sa « suppression » ne ferait que disperser sa logique et casser la dérivation d'univers. Cible long terme inchangée, mais sans déclencheur réel.
...
**Les deux gros mouvements, re-réfutés.** Convergence des _signatures_ de service (A#1) : **nuisible** — l'alias public **v2 `if`** d'event-search, ~26 consommateurs internes, pièges d'auto-ajouts. Mappers projection-aware (C) : **net négatif** — multiplie la complexité, casse la dérivation d'univers, perd sa couverture, sans perf. Aucun n'est « architecture pour la symétrie » qu'on doive forcer.

**Verdict final vs review du 19 juin.** L'expérimentation **confirme et affine** la décision de différer : (a) corrige une erreur — `canEditEvent` est **fail-closed**, pas fail-open (le risque était fonctionnel, pas une faille) ; (b) ajoute le **bloqueur dur** que le 19 juin n'avait pas vu — l'alias public v2 `if` rend le renommage d'event-search non-cosmétique ; (c) **chiffre** le négatif de Lot C (−53 lignes centralisées contre +86 conditionnels distribués + 1 dépendance cassée). Le **seul** incrément sûr et de valeur du « bottom-up » était la synthèse de _frontière_ — désormais implémentée. Le reste (chemin **II** + Lot C) attend un déclencheur réel, pas un principe.
