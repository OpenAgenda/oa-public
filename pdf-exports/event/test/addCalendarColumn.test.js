...
async function addTestCaseTitle(doc, cursor, content) {
  await addText(doc, cursor, { content, fontSize: 10 });
  cursor.y += 30;
}

const doc = new PDFDocument({ size: 'A4', margin: 0 });
const writeStream = fs.createWriteStream(`${pdfTestFolder}/addCalendarColumn.pdf`);
...
await addTestCaseTitle(doc, cursor, "All dates fit in 1 column");
height = doc.page.height;
isMonthFullyProcessed = true;
await addCalendarColumn(doc, cursor, timings, isMonthFullyProcessed, { availableSpace, columnNumber, height, lang }); 

cursor.y += 30 

await addTestCaseTitle(doc, cursor, "Some dates don't fit in 1 column");
height = cursor.y + 100;
isMonthFullyProcessed = true;
await addCalendarColumn(doc, cursor, timings, isMonthFullyProcessed, { availableSpace, columnNumber, height, lang }); 
...
await addTestCaseTitle(doc, cursor, "Month header not displayed");
height = cursor.y + 100;
