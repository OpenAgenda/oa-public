(function() {var loadJs=function(a,b){if(typeof a=='string'){var c=document.createElement('script');if(c.readyState){c.onreadystatechange=function(){if(c.readyState=="loaded"||c.readyState=="complete"){c.onreadystatechange=null;if(typeof b=="function")b();b=null}}}else{c.onload=function(){if(typeof b=="function")b();b=null}}c.charset="utf-8";c.src=a;c.type='text/javascript';document.getElementsByTagName('head')[0].appendChild(c)}else{var d=0;for(var i=0;i<a.length;i++){loadJs(a[i],function(){d++;if(d==a.length){b();b=null}})}}};
  ...
  var handleSuggestions=function(inputElem,list,key,template,options){var contextMenu,contextDiv,possibles=[],run=function(){options=extend({contextMenuClass:false,onSelect:false,onChange:false,maxResults:10,filter:false,invalidClass:'invalid',match:'loose',},options);contextDiv=document.createElement('div');contextDiv.style.display='none';if(options.contextMenuClass)addClass(contextDiv,options.contextMenuClass);inputElem.insertAdjacentElement('afterend',contextDiv);contextMenu=handleContextMenu(inputElem,contextDiv,sEventHandler.getInstance(),{openOnClick:false});addEvent(inputElem,'keyup',function(e){removeClass(inputElem,options.invalidClass);if(e.keyCode==13)if(possibles.length)return _select(possibles[0]);if(options.onChange)options.onChange(inputElem.value);possibles=_shortlist(inputElem.value,list,key);if(possibles.length===0||possibles.length>options.maxResults){if(!possibles.length)addClass(inputElem,options.invalidClass);contextMenu.hide();return}_writeSuggestions(contextDiv,possibles,template,_select);contextMenu.show()})},_writeSuggestions=function(canvasElem,list,template,onClick){while(canvasElem.hasChildNodes())canvasElem.removeChild(canvasElem.childNodes[0]);var ejs=new EJS({text:template}),newChild,ul=document.createElement('ul'),li;forEach(list,function(listItem){li=document.createElement('li');li.innerHTML=ejs.render(listItem);addEvent(li,'click',function(){onClick(listItem)});ul.appendChild(li)});canvasElem.appendChild(ul)},_shortlist=function(value,list,key){var regex=_buildRegex(value),selection=[];forEach(list,function(listItem){if(listItem[key].match(regex))selection.push(listItem)});return selection},_buildRegex=function(value){var regex='';var clean=value.toLowerCase();if(options.filter)clean=options.filter(clean);if(options.match=='direct'){regex=clean.toLowerCase()}else{forEach(clean.toLowerCase(),function(c){regex+='.*'+c})}return new RegExp(regex)},_select=function(selectedItem){inputElem.value=selectedItem[key].replace('&#039;','\'');contextMenu.hide();if(options.onSelect)options.onSelect(selectedItem)},_remove=function(){if(contextDiv)contextDiv.parentNode.removeChild(contextDiv)};run();return{remove:_remove}},
  ...
  onLoad = function(element, register) {
    ...
    var cibulSearchWidget = cibulWidget({
      ...
      init: function(ctl, config) {
        ...
        handleSuggestions(el(element, 'input'), this.sIndex, 'name', '<div><%= label %></div>', {
      ...
      clean: function(name) {
        ...
        clean = this.removeDiacritics(clean);
      ...
      index: function(name, location, id) {
        ...
        if (typeof this.sIndex[cleaned] == 'undefined') {

          this.sIndex[cleaned] = {ne: [coords[0]+this.offset, coords[1]+this.offset], sw: [coords[0]-this.offset, coords[1]-this.offset], score: point, label: name };

          // if this is a place, keep track of id to throw it in the callback
          if (id) this.sIndex[cleaned].id = id;

          return;

        } else {

          if (typeof this.sIndex[cleaned].id !== id) this.sIndex[cleaned].id = undefined;

        }

        if (this.sIndex[cleaned].ne[0] < coords[0]) this.sIndex[cleaned].ne[0] = coords[0] + this.offset;
        if (this.sIndex[cleaned].ne[1] < coords[1]) this.sIndex[cleaned].ne[1] = coords[1] + this.offset;
        if (this.sIndex[cleaned].sw[0] > coords[0]) this.sIndex[cleaned].sw[0] = coords[0] - this.offset;
        if (this.sIndex[cleaned].sw[1] > coords[1]) this.sIndex[cleaned].sw[1] = coords[1] - this.offset;

        this.sIndex[cleaned].score += point;
