...
| Fichier                            | uid            | Contenu                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ---------------------------------- | -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`mcp-usage.json`](mcp-usage.json) | `oa-mcp-usage` | Usage applicatif : `execute` calls/s par `outcome`, ratio d'erreur, latence p50/p95/p99, in-flight, hit-ratio + spares du warm-pool, `search_docs` calls/s, **RAM + CPU pic par µVM** : _host_peak_ / _cpu_ (empreinte hôte réelle = `VmHWM` + `utime+stime` du VMM libkrun) + _workload_peak_ / _workload_cpu_ (au-dessus du baseline boot ≈ ce que le run a touché), **rejets de concurrence** par `reason` (`queue_full` / `timeout` / `shutting_down` du cap global, `caller_cap` du cap par-caller), **churn du warm-pool** (created / failed / expired), **ratio d'échec réel vs backpressure** (deux panneaux distincts : les caps/limites qui s'engagent — `caller_busy` / `rate_limited` / `busy` — ne sont plus comptés comme des « erreurs ») et **occupation par-caller à l'admission** (histogramme **lossless** : combien de runs un caller tenait déjà au démarrage d'un run — capte les pics courts qu'une jauge échantillonnée à l'intervalle d'export raterait). Source = les métriques OTel `oa_mcp_*` (voir `packages/mcp/src/metrics.js`). |
