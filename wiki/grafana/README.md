...
| Fichier                            | uid            | Contenu                                                                                                                                                                                                                                  |
| ---------------------------------- | -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`mcp-usage.json`](mcp-usage.json) | `oa-mcp-usage` | Usage applicatif : `execute` calls/s par `outcome`, ratio d'erreur, latence p50/p95/p99, in-flight, hit-ratio + spares du warm-pool, `search_docs` calls/s. Source = les métriques OTel `oa_mcp_*` (voir `packages/mcp/src/metrics.js`). |
...
> `oa_mcp_concurrency_inflight`, etc. Si ta chaîne d'ingestion nomme autrement
