...
| Fichier                            | uid            | Contenu                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ---------------------------------- | -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`mcp-usage.json`](mcp-usage.json) | `oa-mcp-usage` | Usage applicatif : `execute` calls/s par `outcome`, ratio d'erreur, **latence p50/p95/p99 par outil** (`execute` + `search_docs`), **taille de réponse p50/p95/p99 par outil**, in-flight, hit-ratio + spares du warm-pool, `search_docs` calls/s, **RAM + CPU pic par µVM** : _host_peak_ / _cpu_ (empreinte hôte réelle = `VmHWM` + `utime+stime` du VMM libkrun) + _workload_peak_ / _workload_cpu_ (au-dessus du baseline boot ≈ ce que le run a touché), **rejets de concurrence** par `reason` (`queue_full` / `timeout` / `shutting_down` du cap global, `caller_cap` du cap par-caller), **churn du warm-pool** (created / failed / expired), **ratio d'échec réel vs backpressure** (deux panneaux distincts : les caps/limites qui s'engagent — `caller_busy` / `rate_limited` / `busy` — ne sont plus comptés comme des « erreurs » ; les panneaux filtrent le **label `error`** `="true"`/`="false"`, dont la classification a une source unique côté code — la table `OUTCOMES` de `server.js` — au lieu de ré-énumérer les outcomes en PromQL) et **occupation par-caller à l'admission** (histogramme **lossless** : combien de runs un caller tenait déjà au démarrage d'un run — capte les pics courts qu'une jauge échantillonnée à l'intervalle d'export raterait). Source = les métriques OTel `oa_mcp_*` (voir `packages/mcp/src/telemetry.js`). |
...
> histogrammes **par-tool** (label `tool` = `execute` / `search_docs`)
> `oa.mcp.tool.duration` (unité `ms`) → `oa_mcp_tool_duration_milliseconds_bucket` et
> `oa.mcp.tool.response_size` (unité `By`) → `oa_mcp_tool_response_size_bytes_bucket`
> (latence + taille de réponse p50/95/99 par outil), gauges `oa_mcp_warm_pool_idle` /
...
> `oa_mcp_warm_pool_expired_total`, et l'histogramme d'occupation par-caller
> `oa.mcp.caller.concurrency_at_admission` → `oa_mcp_caller_concurrency_at_admission_bucket`
> / `_sum` / `_count`, etc. Si ta chaîne d'ingestion nomme autrement
...
> **Le « qui » (par-caller) n'est PAS dans ces métriques — c'est volontaire.** Aucune
> métrique ne porte de label `sub`/`callerId` : le `sub` OAuth est non borné, donc en
> faire un label Prometheus ferait exploser la cardinalité. Grafana donne les
> **tendances agrégées** (taux de rejet `caller_cap`, ratio backpressure, occupation
> à l'admission) ; pour savoir **quel** caller a tapé le plafond, le forensique
> par-événement est dans l'**audit log** (InsightOps), qui horodate chaque `execute`
> avec `outcome` + `callerId`/`callerUid` (`outcome=caller_busy` groupé par caller).
> Idem pour le timing fin : l'export OTel (cadence `OTEL_METRIC_EXPORT_INTERVAL`,
> 15 s par défaut) lisse un burst sub-15 s dans son intervalle — le total est exact,
> le timing précis est dans l'audit log.
