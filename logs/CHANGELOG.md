...
- [`0e637d9`](https://github.com/OpenAgenda/oa/commit/0e637d97919b2e83de5a7d9e3216bf3fd8dcf2f9) Thanks [@bertho-zero](https://github.com/bertho-zero)! - Add OpenTelemetry support: a new `OtelTransport` enabled via the `otel` option, and an `includeSpan` option on `DebugTransport` (defaults to `true`). Error serialization helpers are extracted to `utils/errorToJSON` and `utils/cloneAndReplaceErrors`.
