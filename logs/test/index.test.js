...
describe('logs', () => {
  ...
  describe('namespaced logger', () => {
    ...
    it('log with falsy ending value', () => {
      ...
      sinon.assert.calledWith(spy, 'info', 'Je veux un zéro comme ça:', {
        namespace: 'test-7',
      });

      log('info', 'Je veux un zéro comme ça: %d', 0);

      sinon.assert.calledWith(spy, 'info', 'Je veux un zéro comme ça: 0', {
        namespace: 'test-7',
      });
    });
    ...
    it('setConfig - set config of a logger', () => {
      ...
      log.setConfig({
        prefix: 'prefix:',
        namespace: log.options.namespace,
        enableDebug: true,
      });
    ...
    it('logs errors stack', () => {
      ...
      log('error', 'On a eu une erreur: %s', new Error('Une erreur ici !'), {
        test: 789,
      });
      log(
        'error',
        'Error with event %s, (%s)',
        'un-event',
        12345678,
        new Error('Une erreur bidon.'),
      );
